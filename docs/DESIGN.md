...
- **热重载这件事也是实测的**：这个 profile 是 `patchReload: live`，改一次 `cordis.patch.yml`
  就会让这一行就地重跑 `apply()`，入口按 `lib/team.js` 的 mtime 重新 import —— 于是"改完不用重启"
  成立；但**只对"有实际差异"的改动成立**（loader 比的是组合后的树），而且换掉 profile 里
  `node_modules/dsh-plugin-team` 的软链指向**必须重启**（入口模块与它算出的实现路径在启动时就定了）。
  两条都在隔离实例里验过，写进了使用说明 §14.1。
