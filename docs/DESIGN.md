...
> ⚠️ **这一版即将被删掉。** 2026-09-12 决定**推倒重来**：目标架构见
> [`ARCHITECTURE-V2.md`](ARCHITECTURE-V2.md)（工作区文件台账为唯一真相、一群一台编排机器人、
> 两道真人门禁、单文件 ≤400 行）。本文件保留下来当**手术前的病历** ——
> 它准确描述了现在跑着的这 25,000 行是怎么组织的、为什么会长成这样；
> 写 V2 的代码时**不要**照它来。
>
> **这份文档和其他文档的关系**
>
> - `docs/REQUIREMENTS.md` 是**要做什么**：改造后的需求（形态约束、可验收条目、明确不做、做不到）。
> - `../team-agent-architecture/` 是**做错的那版需求**（载体被写成自建 Hub 平台）：**已作废、仅作取证**，
>   不要当规格读。它写对的"团队语义"那一层（对象、状态机、门禁、租约、角色域、播报策略）已经搬进
>   `lib/domain/` 并在本文里有准确描述。
> - `docs/GAP-VS-DESIGN.md` 是**历史**：与旧需求的差距清单，以及每一批改了什么、为什么。
> - `docs/DESIGN.md`（本文）是**现在是什么**：把已经落地的代码反过来写成一份可以照着读、照着改、
>   照着接手的架构说明书。凡是本文写下的行为，都能在代码里指到位置；凡是代码里刻意不做的事，
>   本文也会说明理由。
> - `team-agent-architecture/09-implementation-status.md` 是**实现真相矩阵**（逐项 ✅/🟡/❌/⛔ 与证据）。
>
> 读法建议：先看 §2 形态与 §3 总览图建立地图，再看 §5 数据模型与 §15 不变量（这两节是"改代码前必须知道的事"），
> 然后按链路读 §6–§12。§21 是本文自己承认的缺口。
...
`test/` **40 个文件约 15.3k 行，534 个用例**（`wc -l` 实测，2026-09-12 会话按机器人分开后）。
...
```
        ...
        │  规范化消息 → 去重(event_id) → 群记录/定主 →             │
        │     表态(reaction) → 资产落库 → 分诊 → 一个机器人回答    │
        └───────┬───────────────────────┬──────────────────┬───────┘
                │                       │                  │
        ┌───────▼────────┐   ┌──────────▼────────┐  ┌──────▼─────────┐
...
**"一个群每台机器人一条自己的会话"是硬规则**（用户 2026-09-12 明确要求）。两条配套：

| 事 | 规则 | 为什么 |
|---|---|---|
| 别的机器人有自己的会话说 | 一律 `team-bot-<机器人>-<群>`，Id 里说清是谁的 | 单助手时代那个 `team-feishu-<群>` 曾经被"第一个服务这个群的机器人"**继承**，画面就是"这个群只有一条会话" —— 继承已拿掉，历史记录由 `migrateLegacySessions()` 一次性改名（记 `migrated_from`，旧会话文件留在盘上，日志里写一行：换 id 等于换了 DSH 会话） |
| 什么时候**登记**这条会话 | 第一次拿到"这台机器人在这个群里"的**证据**时：它的应用收到过消息，或消息 @ 到了它（`mentionedAppsIds`）。**真正能跑的 DSH 会话**在它第一次回答时才打开 | 台账与侧栏上"有没有会话"是看得见的：等它开口才登记，画面是"群里两台机器人、会话页只有一台"，与"共用一个会话"一样容易被误读。而"证据"是硬的：不按名册凭空建 |
| 谁记消息数 | 仍然只记在**主机器人**名下（§4.4） | 每台都记一遍会让 N 台机器人的数字永远一样，"这个群归谁记"就没有意义了 |
...
| 事实 | 从哪来 | 谁能用它 |
|---|---|---|
| 这个事件从**哪个应用**来的 | 事件被哪条连接收到（`__appId`） | 路由的**兜底**依据 |
| 这条消息**@ 到了谁** | `mentions[].id.open_id`（`normalizeMessage` → `mentionedOpenIds`） | 路由的**第一依据** |
| 某个应用的机器人 **open_id** | 它自己的连接问 `/open-apis/bot/v3/info`（`configuredBotOpenId`：安装级 `feishu.botOpenId` **只属于默认应用**） | 把"@ 到了谁"翻译成"哪个应用" |

一个群里有两台机器人（各自一个应用）时，**同一个群消息会在两条连接上各到一次**，去重只放先到的那条过去 ——
于是"事件从哪个应用来"是**竞态**，而"@ 到了谁"是**事实**。路由必须先按事实走：
`team.js` 的 `mentionedAppsOf` 把名单翻成应用清单，`routeBots` 因此**不会**把被点名的那个应用的机器人
排除在外（它排候选第一名），`responder.addressedTo` 也认这份清单而不是正文里的名字。

⚠️ 2026-09-12 的线上形状：`configuredBotOpenId` 之前对所有应用都回落到安装级 `feishu.botOpenId`，
于是第二台机器人顶着第一台的 open_id —— @ 它永远不算"被点名"（`requireMention` 默认 true → 永远不开口），
反过来 @ 第一台时它倒以为自己被点名。`test/multi-app-identity.test.mjs` 与
`test/bot-routing.test.mjs` 把两头都钉住了。
...
**但要分清楚冻的是什么**：冻结的是"**谁必须点头**"（`required_by`，含"谁是替身"`stand_ins`）与"超时怎么办"
...
**`stand_ins`（R5.6）**：某个域一个人都没有时，验收门禁由配置的 `domainFallbacks[域]` 顶上。
顶上的人**不是**那个域的负责人，所以建任务时把"谁是替身"记进门禁 —— 卡片据此把那个"代"字
打出来（`@X 代确认`），否则人分不清 `@X` 是本域负责人还是替一个空域签字的替身。
...
2. **准入**（可选收紧，`lib/bots.js` 的 `isRegisteredChat`）：`feishu.requireRegisteredChat` 打开、
   或写了 `feishu.chatAllowlist` 时，没登记的群到此为止 —— **连收件箱都不进**，日志里说明为什么。
   **位置在认领之前**：准入是"要不要收"，判重是"收过没有"，前者必须在外层。
   写反了的后果不是脏数据那么轻 —— 把群加进白名单之后，飞书重投的那条消息会被判成"重复"，
   于是"我加白名单了它还是不理我"（`test/invariants.test.mjs` 钉住这一条）。
3. **占位认领**（`inbox.claim()`，`lib/team.js`）：`event_id` 是权威键（重投时 `message_id`
...
4. **群记录与定主**：写入/更新 `chat`（`app_id`、`primary_bot_id`、`messages`、`last_inbound`），
   并把这行记在主机器人名下（`botsession.seen`）。这一步在**任何回答之前**发生。
5. **资产落库**（`lib/assets.js`）：图片/文件下载 → `assets/<id>/<原名>` → `asset://<id>`；
   `asset-ref:` 占位替换成真引用。**下载失败就没有引用**（指向空气的引用比"没收到"更糟），
   失败逐条写日志。这一步站在"记录是地板"一侧：流水线没起来也照做。
6. **表态**：加一个 reaction（`feishu.reaction`，默认开；`feishu.reactionScope` 默认 `all`
   = 每一条收到的消息，`addressed` = 只对 @ 了机器人的）。**表态 ≠ 会建单，也 ≠ 会回答**，
   它是"我看到了"的唯一即时反馈 —— `requireMention: true` 的群里机器人只在被 @ 时开口，
   没有它，说话的人只能猜它是不是掉线了。失败只记日志。
   **位置紧跟在群记录之后、资产下载之前**：下载一张大图要几秒，表态排在后面就从
   "我看到了"变成"下载完成了"（`test/reaction.test.mjs` 用调用顺序钉住这一条）。
...
**确认行**（`taskConfirmLine`）是"这道门禁现在怎么样"的唯一出口，三样东西必须在上面：
等谁（`gatePending`）、**等了多久**（`已等 X`，起点 = 任务进入当前状态的那一刻，
没有历史就整段不写 —— 与 footer 同一条"不谎报"的规矩）、**谁在代确认**
（`stand_ins` 里的名字写成 `@X 代确认`）。"慢"和"死"在卡片上一模一样，是这类卡最要命的地方。
...
面向"用这套东西的人"（不是改代码的人）的说明书是 `docs/USER-GUIDE.md`：
装与开、第一次配置、群里怎么用（命令总表 + 卡片怎么读）、一条需求的完整旅程、
面板六个页签、数据在哪、文档与记忆、Git/CI、权限速查、常用配置、**排障手册**、做不到的事。
本文（设计文档）与它的分工：本文回答"为什么这样"，说明书回答"我该怎么做"。
...
| 15 | 表态**同步发起、不 await**，且排在资产下载之前 | 那个表情从"我看到了"变成"下载完成了"；或者一次接口抖动把整条入站链路拖住（`test/reaction.test.mjs` 用调用顺序钉住前者） |
| 16 | 身份按应用解析：安装级 `feishu.botOpenId` 只属于默认应用；「@ 到了谁」按 open_id 认、不按名字猜 | 第二个应用顶着第一个的身份：@ 它永远不算被点名（于是不开口），@ 第一台时它却以为自己被点名（§4.5） |
...
| 17 | **准入（要不要收）排在判重（收过没有）之前** | 不在册的群也留下一条认领记录；把它加进白名单后，飞书重投的那条消息被判成"重复"而永远进不了流水线（`test/invariants.test.mjs`） |
| 18 | 租约状态只能由 `lib/domain/lease.js` 的函数构造（`releaseLease`/`returnLease`/`noticeLeaseExpiry`） | 调用点各写一份 `{ ...lease, state: … }`，"什么原因对应什么状态"就散在四处，改一处忘一处（`test/invariants.test.mjs` 扫源码） |
...
| 表态（reaction）失败/没权限 | 只写一行 warn（日志页 `reaction` 源），**不重试**；消息照常记录、照常分诊 |
...
- **零依赖**：`npm test` = `node test/run-all.mjs`，39 个文件 534 个用例，跑的是真实模块（不是 mock 世界）。
...
- **热重载的边界是实测出来的，写在使用说明 §14.1**，一句话概括：**只有 `lib/team.js` 能热重载，
  `lib/` 下其它文件必须重启**。原因不是配置，是 Node 的 ESM 缓存按 URL 命中 ——
  入口 `import('./team.js?v=<mtime>')` 每次都拿到新模块，而它内部的 `import './bots.js'`
  是普通说明符、URL 不含版本号，于是依赖停在**进程启动那一刻**的版本（同一进程内的实测：
  改了依赖再按新 URL 载入父模块，父模块里读到的仍是旧依赖）。入口 `lib/index.js` 本身同理：
  loader 按路径导入、进程内只导一次，重载重跑的是启动时那一版。
