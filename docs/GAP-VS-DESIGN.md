...
### 第五批：卡片与渲染的零碎 + 入站非文本（2026-09-12）

| 修的 | 之前 | 现在 |
|---|---|---|
| **footer 三字段** | `card.footer` 只有耗时/token 的**位置**，但 (a) 原生卡片 JSON 整段漏了它，(b) 没人往里放数据 | `buildCardJson` 末尾渲染 `note`；`run_task` 把**这一轮的事实**（`elapsed_ms` / `steps` / `tokens`）写进会话记录，卡片按 `ctx.runOf(taskId)` 取用；`tokenMeter` 是可选服务，量不到就是 `null` |
| **不谎报 0** | — | `footerOf(task, run)` 逐段"有才写"：provider 没报 token 就没有 🧠 那一段；估算值带 `≈`；超时写成 `⏱ 1m00s（超时）` |
| **表格单元格** | 单元格里的换行/制表符会把等宽文本的行拆散、把列宽搞乱，`\|` 在 markdown 降级里会被当成列分隔符 | `cleanCell()` 在两种渲染之前都跑一遍：换行→空格、tab→空格、`\|` 转义、压缩连续空格 |
| **真实 @** | 卡片里写"@张三"只是文字，**飞书不会通知任何人** | 卡片用 `<at id=ou_x></at>`（open_id 从成员表拿，拿不到退回名字），纯文本降级自动换成 `<at user_id="ou_x"></at>` —— 两种语法不通用，写错了不报错、只是没人被通知 |
| **话题隔离** | `reply_to` 的管道一直在（`cards.js` 支持引用回复并自动退化），但没有任何调用点 | 入站的三处回复（追问 / 重复提示 / 已记为需求）都引用触发它的那条消息；`sendThrough(chatId, ladder, {replyTo})` 引用失败自动退回普通消息（答案不能被引用吞掉） |
| **富文本 `post`** | 读出来是**空字符串**：带标题、链接、图片排版的说明在插件看来等于没说话 | `richTextToMarkdown()`：段落×行内的二维结构压成 markdown；机器人自己的 @ 仍然删掉（那是"在叫它"）；图片留 `asset-ref:` 占位，落库后替换成真引用 |
| **图片/文件** | 落库但 `raw_content:''`，`image_key` 只在消息范围内有效 → 事后谁也取不回来 | `lib/assets.js` + `client.download()`：落到 `assets/<id>/<原名>`，索引 `assets/index.jsonl`；引用是 `asset://<id>`；**下载失败就不给引用**（指向空气的引用比"没收到"更糟）；群里回执"已收到图片"走通知器（节流/digest 规则共用） |
| **去重键** | 只认 `message_id`，而设计 04 §4.1 写明权威键是 `event_id`（重投时前者会变） | `Inbox` 维护 `event_id → message_id` 索引；**地板那一层**先查重（在下载、落盘、回执之前）；保留窗口 `feishu.dedupeRetentionDays`（默认 30 天，`prune()` 同时重写文件与索引） |
