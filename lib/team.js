...
import { gateSatisfied } from './domain/index.js'
import { createLogBus } from './logbus.js'
import { createMetrics } from './metrics.js'
import { FeishuClient, resolveCredentials } from './feishu/client.js'
import { admissionRequired, findBot, isRegisteredChat, pickPrimaryBot } from './bots.js'
import { appDescriptors, appIdOfBot, describeApp, unbootableBots } from './feishu/apps.js'
import { ensureBotSession, migrateLegacySessions, noteMessage } from './sessions.js'
...
/** 工作区文档库（设计 01 §4）：frontmatter 是硬要求，索引可重建，陈旧可检测。 */
export let docs = null

/**
 * 测试接缝：让用例能喂一条伪造的飞书事件走真实的入站路径（`test/wiring.test.mjs`）。
 * 生产代码不读它，它只是一条"能测"的通道（与 `clientFor` 同一个理由）。
 *
 * **它为什么在模块作用域，而不是 `ctx.teamFeishu`。** 这里付过一次线上事故的学费，值得写清楚：
 * Cordis 的 `ctx` 是带守卫的代理 —— 给未 `provide` 的属性赋值，在**运行中的 fiber** 里会抛
 * `cannot set property "X" without provide`（只有 `ctx.fiber.runtime` 为空时才放行，所以
 * `new Context()` 这种裸上下文反而不抛，更难在本地复现）。而测试替身是个普通对象，赋值当然成功。
 * 后果是测试全绿、真宿主里 `apply()` 在飞书长连接启动**之前**就抛出：长连接没起、三条路由没挂，
 * 而浏览器半边是静态加载的，面板照常渲染 —— 使用者看到的是一个 404 的台账页，看着像路由写错了。
 * 接缝挪到模块作用域后，生产路径不再往 `ctx` 上写任何东西（同一条教训的另一半写在
 * `lib/index.js` 的 `mountRoutes` 上：boot 早期用 `ctx.get` 探针会 miss，必须用 `ctx.inject`）。
 *
 * `test/activation.test.mjs` 用**真 Cordis 上下文**跑一遍 `apply()`，专门守住这一类错误：
 * 只要有人再往 `ctx` 上写一个未声明的属性，那条用例就会红。
 */
export let feishuSeam = null
...
async function sendDailyReports({ notifier, store, logbus, day }) {
  ...
  for (const task of store.all('task')) {
    /*
     * ⚠️ `gates` 是**一张表**（`{accept: {…}, start: {…}}`），不是数组，而且 Gate 上没有
     * `name`/`state` 字段（状态是 `gateSatisfied()` 算出来的）。这段以前写成
     * `Array.isArray(task.gates)`，于是**整段是死代码** —— 日报里"现在有什么在等你"
     * 从来没出现过，而设计里"人不在时补发"就靠它。
     */
    const gates = task.gates !== null && typeof task.gates === 'object' ? task.gates : {}
    const open = []
    for (const [name, gate] of Object.entries(gates)) {
      if (gate === null || typeof gate !== 'object') continue
      if (gate.not_applicable === true) continue
      if (gate.cancelled_at !== undefined && gate.cancelled_at !== null) continue
      /*
       * 手上这个对象可能是**手写或旧版**的（缺 `confirmed_by`）：补全字段再判，
       * 不要让一条字段缺失把整份日报变成一句异常。
       */
      const normalized = { required_by: [], confirmed_by: [], not_applicable: false, ...gate }
      if (normalized.required_by.length === 0 && normalized.confirmed_by.length === 0) continue
      if (gateSatisfied(normalized)) continue
      open.push({ name, who: Array.isArray(gate.required_by) ? gate.required_by : [] })
    }
    if (open.length === 0) continue
    const chatId = chatIdOfTask(store, task)
    if (chatId === null) continue
    const waiting = open
      .map((gate) => gateLabelOf(gate.name) + '：等 ' + (gate.who.length > 0 ? gate.who.join('、') : '（没有确认人）'))
      .join('；')
    const list = byChat.get(chatId) ?? []
  ...
  for (const [chatId, lines] of byChat) {
    ...
    const result = await notifier.report({
      ...
      botId: primaryBotIdOfChat(store, chatId),
    })
...
/** 门禁的中文名（日报里给人读）。 */
function gateLabelOf(name) {
  if (name === 'confirm_split') return '确认拆解'
  if (name === 'accept') return '接受'
  if (name === 'start') return '开始'
  if (name === 'acceptance') return '验收'
  return String(name)
}

/** 本地日期（yyyy-mm-dd）：与 `feishu.dailyReportHour` 的本地小时同一个时区。 */
 ...
 *
 * **与 inbound 路径共用同一套解析**，包括 `bot_id` 那条历史兼容：
 * 旧群记录里那个字段叫 `bot_id`（谁回答谁被记下），真实数据目录里两个群都还是旧形状 ——
 * 只读 `primary_bot_id` 的话，日报会落到 `notify` 的 `'team'` 兜底桶里，
 * 于是"由默认应用的脸发出去"，多应用安装下看起来像换了个机器人在说话。
 */
function primaryBotIdOfChat(store, chatId) {
  const chat = store.get('chat', chatId)
  if (chat === null || chat === undefined) return undefined
  if (typeof chat.primary_bot_id === 'string' && chat.primary_bot_id !== '') return chat.primary_bot_id
  if (typeof chat.bot_id === 'string' && chat.bot_id !== '') return chat.bot_id
  return undefined
}
...
function createFeishuController({ ctx, config, store, handlers, pool, notify, logbus, assets }) {
  ...
  /**
   * 这条消息要不要表态（`feishu.reactionScope`）。
   *
   * 发送者不用在这里判：`normalizeMessage` 已经把"机器人自己发的"和
   * `sender_type: 'app'` 挡在外面了 —— 否则机器人会给自己的每一张卡片都点一个表情。
   * 去重也不用在这里判：这一步跑在 `inbox.claim()` 之后，重投的消息根本走不到这里。
   */
  function shouldReact(message, config) {
    if (config.feishu === undefined || config.feishu === null || config.feishu.reaction === false) return false
    if (String(message?.messageId ?? '') === '') return false
    if (config.feishu.reactionScope === 'addressed' && message?.addressed !== true) return false
    return true
  }

  /**
   * 表态：给这条消息加一个 reaction，失败只记日志。
   *
   * 用 `clientForChat`（这个群平时跟哪个应用说话）而不是"哪个连接收到了这条"：
   * 一个群里两台机器人时收件是并发的、谁先到不确定，而表情是**给全群看的**，
   * 它应该始终由同一个身份来点，否则同一件事会一会儿这张脸、一会儿那张脸。
   *
   * **不 await**：表态是旁路。它慢了不该拖住记录与分诊，它挂了更不该 —— 一次
   * 飞书接口的抖动不值得让整条入站链路停在那儿。返回值仍然交出去，用例可以直接 await
   * 它，于是"点没点、点的什么"是可测的，而不是只能靠等。
   */
  function reactToMessage(message, config) {
    const reactor = clientForChat(message.chatId)
    if (reactor === null || reactor === undefined || typeof reactor.call !== 'function') {
      return Promise.resolve({ ok: false, code: 'no-client' })
    }
    /*
     * **同步发起**，不要再包一层 `Promise.resolve().then(...)`。
     *
     * 包一层看起来无害，实际是把它推到了下一个微任务，而紧跟其后的 `collectAssets`
     * 是在**同一个 tick 里同步起跑**的（它要先调 `download`，再 await 结果）——
     * 于是那个表情会排在一次大图下载的后面，从"我看到了"变成"下载完成了"。
     * 这条顺序是 `test/reaction.test.mjs` 里 `表态排在资产下载之前` 抓出来的。
     */
    let pending = null
    try {
      pending = reactor.call('/open-apis/im/v1/messages/' + encodeURIComponent(message.messageId) + '/reactions', {
        method: 'POST',
        body: JSON.stringify({ reaction_type: { emoji_type: String(config.feishu?.reactionEmoji ?? 'Get') } }),
      })
    } catch (error) {
      logbus.warn('reaction', '表态异常（不影响回答）：' + String(error && error.message ? error.message : error))
  ...
  /**
   * 这些应用里，**服务这个群的**启用机器人有哪些。
   *
   * `chats` 为空的机器人在它应用的所有群里都服务（这是默认），非空则是一道真过滤。
   * 只看"应用在这个群里出现过"这个证据，不做任何猜测 —— 见 `handleInbound` 里的用法。
   */
  function enabledBotsOf(appIds, chatId) {
    const wanted = (Array.isArray(appIds) ? appIds : []).filter((one) => typeof one === 'string' && one !== '')
    if (wanted.length === 0) return []
    const out = []
    for (const bot of Array.isArray(config.bots) ? config.bots : []) {
      if (bot.enabled !== true) continue
      if (!wanted.includes(appIdOfBot(bot, config))) continue
      const chats = Array.isArray(bot.feishu?.chats) ? bot.feishu.chats : []
      if (chats.length > 0 && !chats.includes(chatId)) continue
      out.push(bot)
    }
    return out
  }
  ...
  /**
   * 这条消息**按 open_id 点名了我们哪几个应用**。
   *
   * 一个应用一条连接、一台机器人一张脸：飞书事件里的 mention 带的是 open_id，
   * 而每个应用自己的 botOpenId 由它自己的连接回答（拿不到就问
   * `/open-apis/bot/v3/info`，见 connection.js 的 `configuredBotOpenId`）。
   * 两边一比对，"被点名的身份属于哪个应用"就是**确定的事实**，而不是
   * "哪条连接先到"那个竞态。
   *
   * 拿不到连接池（`feishu.mode: 'off'`、还没拨号、或 headless）时返回空数组：
   * 这时行为与改动前完全一样（只按收到的那条连接路由），不会凭空多出候选。
   */
  function mentionedAppsOf(message) {
    const ids = Array.isArray(message?.mentionedOpenIds) ? message.mentionedOpenIds : []
    if (ids.length === 0 || state.connectionPool === null) return []
    const out = []
    /*
     * 按**应用描述**遍历，不按 `state.clients`：后者在 `feishu.mode: 'off'` 或还没拨号时
     * 是空的（`start()` 早退），用它会让这条判断在"还没连上"时静默失效 ——
     * 而"哪条连接先到"的竞态恰恰在最开始那几条消息上最要命。
     * 身份以连接池探测到的为准（那是每个应用自己回答的），配置里写死的值兜底。
     */
    for (const descriptor of appDescriptors(config)) {
      const own = typeof descriptor.botOpenId === 'string' ? descriptor.botOpenId : ''
      const probed = state.connectionPool.botOpenIdFor(descriptor.appId)
      const botOpenId = typeof probed === 'string' && probed !== '' ? probed : own
      if (botOpenId !== '' && ids.includes(botOpenId)) out.push(descriptor.appId)
    }
    return out
  }

  async function handleInbound(raw) {
    if (raw.__membership === 'added' || raw.__membership === 'removed') {
      /*
       * 入群/退群都要留痕，而且都值得**明确地**说一句：前者回答"它为什么一直很安静"
       * （它根本没被拉进去），后者回答"它怎么不理这个群了"（它被移出去了）。
       * 退群时顺手把群记录标一下，面板的「群与主机器人」就能显示"机器人已不在这个群"。
       */
      const chatId = raw?.chat_id ?? raw?.event?.chat_id ?? null
      const added = raw.__membership === 'added'
      logbus.info('inbound', (added ? '被拉进一个群：' : '被移出一个群：') + JSON.stringify(raw).slice(0, 200))
      if (!added && typeof chatId === 'string' && chatId !== '') {
        const chat = store.get('chat', chatId)
        if (chat !== null) store.put('chat', { ...chat, bot_removed_at: new Date().toISOString() })
      }
      return
    }
     ...
     * 又会变成一个只能读代码才知道答案的问题。
     *
     * **位置在去重认领之前**，这是修掉的一个真问题：以前它排在 `inbox.claim()` 后面，
     * 于是"不处理"也留下了一条副作用（收件箱里多一行）。后果不只是脏数据 ——
     * 把群加进白名单之后，飞书**重投**那条消息会被判成"重复"而永远不进流水线，
     * 表现就是"我加白名单了它还是不理我"。准入是"要不要收"，判重是"收过没有"，
     * 前者必须在外层。
     */
    if (admissionRequired(config)) {
      /*
       * **每一条消息都判**，不再只是"这个群还没有记录时判一次"。
       * 以前那个 `chat === null` 的短路，让"我把它从白名单里拿掉了"这种事
       * 对已经聊过的群完全无效 —— 白名单形同虚设。
       */
      ...
      if (registered.ok !== true) {
        logbus.warn(
          'inbound',
          '忽略了不在准入范围内的群：' + message.chatId + '（' + String(registered.reason ?? 'unregistered') +
            '：把它加进某个启用中机器人的 feishu.chats，或写进 feishu.chatAllowlist，或关掉 feishu.requireRegisteredChat）',
        )
        return
      }
    }
     ...
     * **别让副作用发生**。两道的顺序不能反。
     */
    const claimed = state.inbox.claim({
      ...
      text: message.text,
      create_time: message.at ?? null,
      received_at: new Date().toISOString(),
    })
    if (claimed.ok !== true) {
      logbus.info(
        'inbound',
        '重投/重复的消息（' + claimed.dedupe_key + ' 命中' + (claimed.duplicate_of === null ? '' : '，原 ' + claimed.duplicate_of) + '）：' +
          message.messageId,
      )
      return
    }
      ...
      chat === null
        ...
        : typeof chat.primary_bot_id === 'string' && chat.primary_bot_id !== ''
          ...
          : // 旧记录里那个字段叫 `bot_id`（谁回答谁被记下）：迁移成"主"。
            ...
            ? chat.bot_id
            : null
    const mentionedAppIds = mentionedAppsOf(message)
    const primary =
      existingPrimary !== null
        ? { id: existingPrimary }
        : pickPrimaryBot(config, { chatId: message.chatId, appId, text: message.text, mentionedAppIds })
    /*
     * 这个群**见过哪些应用**：一个群里两台机器人时，`app_id` 以前写的是"这条消息从哪条
     * 连接来的"—— 而那是竞态（两条连接都会收到，谁先到谁写）。于是这个群的播报身份会
     * 随消息一会儿这张脸、一会儿那张脸。改成：优先**主机器人的应用**（前提是它确实来过
     * 这个群，见 `app_ids`），拿不准时才用收到这条连接的那个。回复不受影响 ——
     * 回答用的是 `clientFor(bot)`（回答者自己的脸）。
     */
    const seenApps = Array.isArray(chat?.app_ids)
      ? chat.app_ids.filter((one) => typeof one === 'string' && one !== '')
      : typeof chat?.app_id === 'string' && chat.app_id !== ''
        ? [chat.app_id]
        : []
    const appIds = seenApps.includes(appId) ? seenApps : [...seenApps, appId]
    const primaryAppId = primary === null ? '' : appIdOfBot(findBot(config.bots, primary.id), config)
    const base = {
      id: message.chatId,
      chat_type: message.chatType,
      app_id: primaryAppId !== '' && appIds.includes(primaryAppId) ? primaryAppId : appId,
      app_ids: appIds,
    ...
    /*
     * 群里每台**确实在这个群里**的机器人各有一份自己的会话（`机器人 × 群`），
     * 现在就把记录建出来 —— 不必等它开口。
     *
     * 为什么要提前建：用户 2026-09-12 的原话是"每个机器人都是单独的会话，不是一个群共用一个"，
     * 而"它有没有会话"在台账和侧栏上是**看得见的**。等它第一次回答才建，画面就是
     * "群里有两台机器人，会话页却只有一台" —— 那和"共用一个会话"一样容易被误读。
     *
     * 证据只有两种（不猜）：这条消息**从哪个应用来**（`appId`），以及它 **@ 到了哪个应用**
     * （`mentionedAppIds`）。两者都意味着那个应用的这台机器人确实在这个群里。
     * 真正能跑的 DSH 会话仍然在它第一次回答时才打开（见 `responder.js`），这里只记台账。
     */
    try {
      for (const bot of enabledBotsOf([appId, ...mentionedAppIds], message.chatId)) {
        if (primary !== null && bot.id === primary.id) continue // 上面已经记过，别把 seen 记两遍
        ensureBotSession(store, {
          botId: bot.id,
          chatId: message.chatId,
          chatType: message.chatType,
          workspace: savedChat.workspace,
      ...
      logbus.error('inbound', '建机器人会话记录失败：' + String(error && error.message ? error.message : error))
    }

    /*
     * 收到消息先表态（设计 04 §0.1）：加一个 reaction。
     *
     * 为什么值得一个 API 调用：**表态 ≠ 会建单，也 ≠ 会回答**，它是"我看到了"的唯一
     * 即时反馈。没有它，一个人在群里说话之后要等模型跑完一轮才知道机器人是否活着 ——
     * 而那一轮可能是几分钟；`requireMention: true` 的群里更是"它只在被 @ 时开口"，
     * 不表态就等于一片沉默。
     *
     * 位置在这一步是**故意的**：紧跟在群记录之后、`collectAssets` 之前。资产下载可能要
     * 几秒（一张大图、一个文件），而"收到"的全部价值就在于它是**立刻**的 —— 放在下载后面
     * 等于让这个表情替下载排队。
     *
     * 范围由 `feishu.reactionScope` 决定（默认 `all`：每一条收到的消息；`addressed`：
     * 只对 @ 了机器人的）。`feishu.reaction: false` 整个关掉。失败只记日志，绝不打断主流程。
     */
    if (shouldReact(message, config)) {
      reactToMessage(message, config)
    }
    ...
    const inbound = { ...message, text, mentionedAppIds }
  ...
  return {
    ...
    logbus,
    /**
     * **活的配置对象**（面板保存会就地重载它，见 `apply()` 里的 `reload`）。
     *
     * 暴露出来有两个用处：诊断时能读到"此刻生效的是什么"，以及测试能验
     * "改了配置之后下一个事件的行为变了没有"（准入那条用例就是这么写的 ——
     * 复制一份配置到测试里等于没测）。
     */
    config,
    /**
...
export async function apply(ctx, rowConfig) {
  ...
  /*
   * 文档载体与回忆（设计 01 §4 / 07 §0.3）：文档的家是工作区里的 markdown，
   * 情景记忆的家是 DSH 自己的会话历史。**不建自建记忆库** —— 会话检索由
   * `sessionQuery` 提供（可选服务，读不到就如实说"这一半没查"）。
   */
  const docStore = createDocStore({ workspace: config.workspace, log: logbus })
  const recall = createRecall({
    docs: docStore,
    store,
    sessionQuery: () => (ctx !== null && typeof ctx.get === 'function' ? ctx.get('sessionQuery') : null) ?? null,
  })
  const handlers = createHandlers({ ctx, config, store, pool, notify: notifier, docs: docStore, recall })
  own(ctx, () =>
    ctx.tools.register(
  ...
  api = createTeamApi({
    ...
    config,
    /*
     * 这个 profile 到底有哪些可选服务。缺失是**静默降级**的：没有 timer 就没有门禁超时
     * 扫描、租约回收、日报与补发；没有 agents 就跑不了任务；没有 connection 就没有面板数据。
     * 面板要能把这件事说出来 —— 否则使用者只会看到"任务怎么不动"。
     */
    capabilities: () => ({
      timer: (() => {
        const service = ctx.get('timer')
        return service !== null && service !== undefined && typeof service.interval === 'function'
      })(),
      agents: ctx.get('agents') !== undefined && ctx.get('agents') !== null,
      connection: ctx.get('connection') !== undefined && ctx.get('connection') !== null,
      sessionQuery: ctx.get('sessionQuery') !== undefined && ctx.get('sessionQuery') !== null,
      tokenMeter: ctx.get('tokenMeter') !== undefined && ctx.get('tokenMeter') !== null,
    }),
  })
  logsApi = createLogsApi({
    logbus,
    store,
  ...
  /** 三个定时器共用的服务：缺它时"到点发生的事"全都不发生，必须说出来。 */
  const timerService = ctx.get('timer')
  const hasTimer = timerService !== null && timerService !== undefined && typeof timerService.interval === 'function'
  if (!hasTimer) {
    logbus.warn(
      'team',
      '这个 profile 没有 timer 服务：门禁/租约超时不会被扫描、被节流并掉的卡片不会补发、日报不会发。' +
        '手动兜底：调 `team` 工具的 `tick`（`dry_run: false` 才真的动状态）。',
    )
  }
  ...
  if (
    ...
    own(ctx, () =>
      reportTimer.interval(() => {
        ...
        /*
         * 用的是**本地日期**，与 `reportHour`（本地小时）同一个时区。
         * 混用 UTC 日期的话：UTC-6 及以西的部署，18:00 发的日报标题会是"明天"。
         */
  ...
  /*
   * 单助手时代的会话记录改名成"机器人 × 群"的 id（幂等，通常一次就迁完）。
   *
   * 说出来，不静默：换 id 就是换 DSH 会话，旧会话的历史不会被带过来（文件还在盘上）。
   * 用户 2026-09-12 的要求是"一个群里每台机器人都是单独的会话"，而会话 id 是**看得见的** ——
   * 留着 `team-feishu-<群>` 会让这句话在界面上看不出来。
   */
  try {
    const migrated = migrateLegacySessions(store)
    if (migrated.length > 0) {
      logbus.warn(
        'session',
        '把 ' + String(migrated.length) + ' 条单助手时代的会话改名为「机器人 × 群」：' +
          migrated.map((one) => one.botId + ' ' + one.from + ' → ' + one.to).join('；') +
          '（旧会话文件仍在 ~/.dsh/sessions 里，历史不会丢；新 id 从下一次回答开始用）',
      )
    }
  } catch (error) {
    logbus.error('session', '会话改名失败（不影响其它功能）：' + String(error && error.message ? error.message : error))
  }
