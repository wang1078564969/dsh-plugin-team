 ...
 */
function createFeishuController({ ctx, config, store, handlers, pool, notify, logbus }) {
  const state = {
...
export async function apply(ctx, rowConfig) {
  ...
  const feishu = createFeishuController({ ctx, config, store, handlers, pool, notify: notifier, logbus })
  ...
  logsApi = createLogsApi({ logbus, store, inbox: feishu.state.inbox, notify: notifier, metrics })
