...
function createFeishuController({ ctx, config, store, handlers, pool, notify }) {
  ...
  async function handleInbound(raw) {
    ...
    /*
     * Remember which app this chat talks to, before anything can answer: every
     * later send for this chat resolves through it, including the immediate command
     * replies that never reach the responder.
     */
    ...
    } else if (chat.app_id !== appId) {
      store.put('chat', { ...chat, app_id: appId, last_seen: message.at ?? new Date().toISOString() })
    }

    const filed = await state.ingest.onMessage(message)
