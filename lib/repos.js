...
/** 仓库的相对路径（面板与日志里显示用；绝对路径对读的人是噪音）。 */
export function relativeRepo(root, file) {
  try {
...
/** 一个目录是不是 git 仓库（决定"能不能检出"这类提示说不说得出口）。 */
export function isGitRepo(root) {
  try {
    return statSync(join(String(root), '.git')) !== undefined
  } catch (error) {
    return existsSync(join(String(root), '.git'))
  }
}
