...
export class FeishuClient {
  ...
  async sendThrough(chatId, ladder) {
    ...
    for (const rung of ladder) {
      const result = await this.send(chatId, rung.payload)
      attempts.push({ level: rung.level, via: rung.via, code: result.code, ok: result.ok })
      if (result.ok) return { ok: true, via: rung.via, level: rung.level, attempts, messageId: result.data?.message_id ?? null }
    }
