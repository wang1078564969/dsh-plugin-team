 ...
 * 只剔"语气与客套"的集合（去重用）。
 *
 * 与下面的 `STOPWORDS` 刻意不同：那个是给**前缀比对**用的，连"支持/需要"一起删也没关系
 * （比的是开头几个字），而整句相似度删掉内容词就等于把句子拆了。
 */
const FILLER = /(能不能|可不可以|是否可以|麻烦|帮忙|帮我|请问|一下|就行|即可|要能|能够|一个|这个|那个|并且|而且|同时|以及|的话|我们|你们|他们)/g
...
export function similarity(a, b) {
  const grams = (s) => {
     ...
     * 比对前只剔**语气与客套**，不剔内容词。
     *
     * 以前这里只去空白，于是"能不能加导出功能，导出 CSV 就行"和
     * "加一个导出功能，要能导出 CSV"的相似度是 **0**：同一件事、只差几个虚词，
     * 去重完全失效。但直接套用 `prefixOverlap` 的 `normalize()` 又太狠 ——
     * 它会把"支持/需要/必须"这类**带意图的内容词**也删掉，两个短标题被削到只剩
     * 骨头之后相似度反而掉下去（"支付要支持重试" vs "支付需要支持重试" 从 1.0 掉到 0.4，
     * 这是被测试抓出来的）。
     *
     * 所以这里用一个只针对客套话的集合：删掉它，句子的意思不变。
     */
    const t = s.replace(FILLER, '').replace(/[\s，,。.、!！?？:：;；]/g, '').toLowerCase()
    const out = new Set()
    if (t.length < 2) {
      if (t !== '') out.add(t)
      return out
