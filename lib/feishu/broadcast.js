...
/**
 * 配额的记录 id：`day|bot|who` → 文件安全的一行。
 *
 * `|` 与 `:` 都不在 `store` 允许的 id 字符里（`safeId`），所以拼的时候换掉；
 * 原始三个字段照旧写进记录，读的时候以它们为准（和 `card` 那条台账同一个做法）。
 */
export function quotaId(dayKey, botId, who) {
  const slug = (value) => String(value).replace(/[^A-Za-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '')
  return [slug(dayKey), slug(botId) || 'bot', slug(who) || 'who'].join('.')
}
 ...
 *
 * **计数落盘**（R13.5）：以前它是纯内存的 Map，进程一重启就当今天没 @ 过人，
 * 于是"一晚上重启三次的开发机"上，配额形同虚设。现在每条计数是一份
 * `quota` 记录（一天一人一行，见 `lib/store.js`），并顺手记下**被静默了几次** ——
 * 那是"配额到底有没有生效"唯一可查的证据。
 *
 * `deps.store` 缺席时退回纯内存（测试与单助手形态都能跑）：
 * 这是**降级**而不是失败 —— 少一个台账不该让播报停下来。
 */
export class MentionQuota {
  ...
  constructor(limitPerDay = 3, deps = {}) {
    this.#limit = limitPerDay
    this.#store = deps.store ?? null
    this.#keepDays = Number.isFinite(deps.keepDays) ? Number(deps.keepDays) : 7
    if (this.#store !== null) {
      for (const row of this.#store.all('quota')) {
        const day = String(row?.day ?? '')
        if (day === '') continue
        const key = `${day}|${String(row.bot ?? '')}|${String(row.who ?? '')}`
        this.#used.set(key, Number(row.used ?? 0))
        this.#silenced.set(key, Number(row.silenced ?? 0))
      }
      this.prune(deps.today ?? null)
    }
  }

  /** 攒一条记录并落盘（`store` 缺席时只改内存）。 */
  #flush(botId, who, dayKey) {
    if (this.#store === null) return
    const key = `${dayKey}|${botId}|${who}`
    this.#store.put('quota', {
      id: quotaId(dayKey, botId, who),
      day: dayKey,
      bot: String(botId),
      who: String(who),
      used: this.#used.get(key) ?? 0,
      silenced: this.#silenced.get(key) ?? 0,
  ...
  tryConsume(botId, who, dayKey) {
    ...
    if (used >= this.#limit) {
      // 超限也要落盘：**被静默的次数**是"配额有没有生效"的证据。
      this.#silenced.set(key, (this.#silenced.get(key) ?? 0) + 1)
      this.#flush(botId, who, dayKey)
      return false
    }
  ...
  /** 被静默（超限后改成静默待办）的次数。 */
  silenced(botId, who, dayKey) {
    return this.#silenced.get(`${dayKey}|${botId}|${who}`) ?? 0
  }

  /** 某一天的全部计数（面板与观测用）。 */
  rows(dayKey) {
    const out = []
    for (const [key, used] of this.#used.entries()) {
      const [day, bot, who] = key.split('|')
      if (dayKey !== undefined && dayKey !== null && day !== dayKey) continue
      out.push({ day, bot, who, used, silenced: this.#silenced.get(key) ?? 0, limit: this.#limit })
    }
    return out.sort((a, b) => b.used - a.used || a.who.localeCompare(b.who))
  }

  /**
   * 丢掉太老的记录（默认留 7 天）。
   *
   * 不清理的话这个目录会一天长十几行、永远不缩 —— 而配额只关心今天，
   * 留一周是为了"昨天是不是也超了"这种追问。
   */
  prune(today = null) {
    if (this.#store === null || today === null) return []
    const cutoff = new Date(String(today) + 'T00:00:00Z').getTime() - this.#keepDays * 86_400_000
    const removed = []
    for (const row of this.#store.all('quota')) {
      const at = Date.parse(String(row?.day ?? '') + 'T00:00:00Z')
      if (!Number.isFinite(at) || at >= cutoff) continue
      this.#store.remove('quota', String(row.id))
      const key = `${String(row.day)}|${String(row.bot)}|${String(row.who)}`
