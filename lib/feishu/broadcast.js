...
export function taskConfirmLine(task) {
  const parts = []
  for (const name of ['confirm_split', 'accept', 'start', 'acceptance']) {
    ...
    if (gateSatisfied(gate)) {
      parts.push(`✅ ${label}已确认`)
    } else {
      const { confirmed, required } = gateProgress(gate)
      const who = gatePending(gate).map(shortName).join(' ')
      parts.push(`⏳ 待${label}确认（${confirmed}/${required}${who === '' ? '' : ` · ${who}`}）`)
    }
  }
