...
export function renderTableAsText(table, maxWidth = 40) {
  ...
  const widths = table.headers.map((h, i) => {
    ...
    return Math.min(maxWidth, Math.max(...cells.map((c) => clip(c, maxWidth).length)))
  })
  const line = (cells) =>
    cells
      .map((c, i) => {
        const text = clip(c ?? '', widths[i] ?? maxWidth)
  ...
  const out = [line(table.headers), widths.map((w) => '-'.repeat(w)).join('  ')]
...
function tableElement(table) {
  return {
    ...
    rows: table.rows.map((row) => {
      const cells = {}
      for (let i = 0; i < table.headers.length; i += 1) cells[`c${i}`] = row[i] ?? ''
      return cells
    }),
  }
}
