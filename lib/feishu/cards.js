...
/**
 * 一个按钮对应的**群命令**（人照着回复就能用的那一句）。
 *
 * 形状与 `lib/feishu/ingest.js` 的 `parseCommand` 一致：`<动词> <对象 id>`。
 * 动词取按钮自己的 label（"接受"/"开始"/"验收"…），对象取按钮 value 里的 `id`；
 * 两者都没有时退回 action 的最后一段，至少不会给出一个绝对解析不了的长串。
 */
export function commandHintOf(button) {
  const label = typeof button?.label === 'string' && button.label !== '' ? button.label : ''
  // `value.id` 是 `buttonValue()` 写的规范字段；其余几个是历史/手写卡片用过的键。
  const value = button?.value !== null && typeof button?.value === 'object' ? button.value : {}
  const id = ['id', 'object_id', 'task_id', 'req_id']
    .map((key) => (typeof value[key] === 'string' ? value[key] : ''))
    .find((one) => one !== '') ?? ''
  /*
   * 需求对象要带"需求"两个字：`废弃` 在任务的词表里是 `drop_task`，
   * 而需求卡上那颗 `req.drop` 是"废弃整条需求"。少了这个区分，人照着回复会
   * 得到一个"按这个 id 找不到任务"的拒绝（卡上写着能做、群里做不了）。
   */
  const forRequirement = value.object === 'requirement' || String(id).startsWith('req-')
  const verb = forRequirement && label !== '' && !/需求|拆解|变更/.test(label) ? label + '需求' : label
  if (verb !== '' && id !== '') return verb + ' ' + id
  if (verb !== '') return verb
  const action = typeof button?.action === 'string' ? button.action.split('.').pop() : ''
  return id === '' ? action : action + ' ' + id
}
...
export function renderCardAsMarkdown(card, opts = {}) {
  ...
  for (const block of card.blocks) {
    switch (block.kind) {
      ...
      case 'buttons':
        /*
         * 兜底文案必须教**群命令真正认的词**：`task.accept` 这种内部动作名
         * 解析器不认识（`parseCommand` 只认动词表），照它回复会被当散文走分诊 ——
         * 可能建出一条垃圾需求。所以这里渲染成「接受 task-1」：
         * 动词用小写中文 label，对象用按钮 value 里带的 id。
         */
        if (withButtons) {
          lines.push(block.buttons.map((b) => `[${b.label}]`).join('  '))
          lines.push('')
          lines.push('（按钮不可用时，直接回复：' + block.buttons.map((b) => '`' + commandHintOf(b) + '`').join('；') + '）')
        } else {
          lines.push('操作：' + block.buttons.map((b) => `回复 \`${commandHintOf(b)}\` 表示「${b.label}」`).join('；'))
        }
        break
      case 'note':
        lines.push(block.content)
    ...
    switch (block.kind) {
      ...
      case 'buttons':
        if (withButtons) {
          ...
          // 同上：教的是群命令真正认的那一句，而不是内部 action 名。
          elements.push({
            tag: 'markdown',
            content: '操作：' + block.buttons.map((b) => `回复 \`${commandHintOf(b)}\` 表示「${b.label}」`).join('；'),
          })
        }
        break
      case 'note':
...
export function degradationLadder(spec, opts = {}) {
  ...
  /*
   * `buttons: false` 必须在**第一级**就生效。
   *
   * 为什么这是硬要求：按钮需要公网 HTTPS 回调，而 DSH Web 只听 127.0.0.1 ——
   * 发出去的按钮**点下去不会有任何反应**。默认配置正是 `buttons: false`，
   * 但这条阶梯以前把它当空气（第 1 级恒 `buttons: true`），于是群里长期挂着一排
   * 死按钮；而"去按钮 + 文本指令"那一级反而永不出现（第 1 级用不着降级）。
   */
  ...
  return [
    { level: 1, via: 'card', payload: cardPayload(true, withButtons) },
    { level: 2, via: 'card-plain-table', payload: cardPayload(false, withButtons) },
