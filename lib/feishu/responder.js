...
export function createResponder(deps) {
  ...
  function pickBot(message) {
    ...
    const boundBotId = chat !== null && typeof chat.bot_id === 'string' ? chat.bot_id : null
  ...
  function addressedTo(message, bot, candidates) {
     ...
     */
    return candidates.length === 1 && candidates[0] === bot && bot.id === store.get('chat', message.chatId)?.bot_id
  }
  ...
  return {
    ...
    async onMessage(message) {
      ...
      if (bot !== null) {
        ...
        store.put('chat', { ...latest, bot_id: bot.id, session_id: sessionId, last_seen: message.at ?? new Date().toISOString() })
      }
