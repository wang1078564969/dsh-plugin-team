  ...
  async function handler(request) {
    ...
    try {
      if (request.method === 'GET') {
        // `?fresh=1` = 显式「重新自检」：绕过自检缓存，真的再打一次飞书。
        const url = new URL(request.url)
        return json(await snapshot({ fresh: url.searchParams.get('fresh') === '1' }))
      }
...
export function createLogsApi(deps) {
  const { logbus, store, metrics, assets, docs } = deps
  ...
  function snapshot(query) {
    ...
    return {
      ...
      assets: typeof assets?.all === 'function'
        ...
        : null,
      /**
       * 文档库（设计 01）：条数、类型分布、有问题的、过期的。**只读的一小块**，
       * 因为"团队记忆现在有多少、哪些过期了"是出问题时最该先看到的一行。
       */
      knowledge:
        typeof docs?.stats === 'function'
          ? {
              docs: docs.stats(),
              stale: typeof docs.staleReport === 'function' ? docs.staleReport({}).rows.slice(0, 10) : [],
            }
          : null,
      counts: {
...
export function createConfigApi(deps) {
   ...
   * 自检结果的短缓存（默认 60 秒）。
   *
   * 自检要**真调飞书**（身份 + 群列表，各 15 秒上限），而面板读这一页只等 20 秒 ——
   * 每次都现调的话，飞书一慢就会看到"读取超时"，让人以为插件坏了。
   * 显式「重新自检」带 `?fresh=1` 绕过缓存；并发请求共用同一个 promise。
   */
  const diagnoseCache = { at: 0, value: null, promise: null }
  const DIAGNOSE_TTL_MS = 60_000
  function cachedDiagnose(input, fresh = false) {
    if (fresh !== true && diagnoseCache.value !== null && Date.now() - diagnoseCache.at < DIAGNOSE_TTL_MS) {
      return Promise.resolve(diagnoseCache.value)
    }
    if (fresh !== true && diagnoseCache.promise !== null) return diagnoseCache.promise
    diagnoseCache.promise = Promise.resolve()
      .then(() => diagnose(input))
      .then((value) => {
        diagnoseCache.value = value
        diagnoseCache.at = Date.now()
        diagnoseCache.promise = null
        return value
      })
      .catch((error) => {
        diagnoseCache.promise = null
  ...
  async function snapshot(options = {}) {
    return {
      ...
      /*
       * 自检要真调飞书（身份 + 群列表，各 15s 上限），而面板的 GET 只等 20s ——
       * 每次都现调的话，飞书一慢就会看到"读取超时"，让人以为插件坏了。
       * 所以结果缓存一小段时间：显式「重新自检」用 `?fresh=1` 绕过缓存。
       */
      diagnostics: await cachedDiagnose({
        config,
      ...
      /**
       * 被 profile 的 row config 固定住的键。面板要提示"改这些不会生效" ——
       * 配置来源是 `env > row > config.json > 默认值`，静默不生效最容易被当成 bug。
       */
      rowPinned: Array.isArray(config.rowPinned) ? config.rowPinned : [],
      /**
       * 这个 profile 有哪些可选服务。缺失是静默降级：没有 timer 就没有门禁超时扫描、
       * 租约回收、日报与补发。面板据此显式提示，而不是让人猜"任务怎么不动"。
       */
      capabilities: typeof deps.capabilities === 'function' ? deps.capabilities() : null,
      editable: [
        'defaultOwner',
        'workspace',
        'workspaceTitle',
