...
/**
 * 表态范围的枚举（`feishu.reactionScope`）。
 *
 * 不认识的值退回 `all`（默认语义），而不是"当作没写"或"猜一个"：配置台保存时
 * 会被 `validatePatch` 挡下来并说清楚，手写文件的人至少拿到的是**默认行为**，
 * 不会因为一个拼错的词得到第三种谁也没想过的语义。
 */
function reactionScopeOf(value) {
  return value === 'addressed' ? 'addressed' : 'all'
}
...
export function loadConfig(rowConfig) {
  ...
  return {
    ...
    feishu: {
       ...
       * 对哪些消息表态。
       *
       * `all`（默认）**每一条收到的消息**都点一下 —— 它是"我看到了"的唯一即时反馈，
       * 而且和"回不回答"是两件事：`requireMention: true` 时机器人只在被 @ 时开口，
       * 但每个群里的人仍然需要知道它到底活着没有（模型跑一轮可能几分钟）。
       *
       * `addressed` 只对 @ 了机器人的消息表态（本插件早期的行为）。热闹的群里
       * 每条消息一个表情会显得吵，那时把它切回 `addressed`。
       */
      reactionScope: reactionScopeOf(pick(rowFeishu.reactionScope, fileFeishu.reactionScope, 'all')),
      /**
