...
export function createMetrics(deps) {
  ...
  function chats() {
    ...
    const rowOf = (chatId) => {
      ...
      const created = {
        ...
        sent: 0,
        received: 0,
        // 卡片原地更新的次数（`deliveries - 1` 的合计）——见下面读卡台账那一段。
        updates: 0,
        mentions: 0,
        silenced: 0,
        suppressed: 0,
    ...
    for (const record of store.all('card')) {
      ...
      row.sent += 1
      /*
       * 这张卡**被更新过几次**（R13.7）：台账里 `deliveries` 数的是"投递了几次"，
       * 第一次是创建、其余是原地更新。这个数字是"同一个话题只有一张卡"的可见证据 ——
       * 它要是塌成 1，说明每变一次状态就在群里多贴一张新卡。
       */
      row.updates += Math.max(0, Number(record.deliveries ?? 1) - 1)
  ...
  /**
   * @人配额：今天谁被 @ 了几次、有几次被改成静默待办（R13.5）。
   *
   * 从**落盘的 `quota` 记录**读（`MentionQuota` 每次计数都写一条），不是从进程内存读 ——
   * 后者重启就清零，而这条指标存在的意义正是"重启之后还算不算数"。
   */
  function quota(day = now().toISOString().slice(0, 10)) {
    const rows = store
      .all('quota')
      .filter((row) => String(row?.day ?? '') === String(day))
      .map((row) => ({
        bot: String(row.bot ?? ''),
        who: String(row.who ?? ''),
        used: Number(row.used ?? 0),
        silenced: Number(row.silenced ?? 0),
      }))
    return {
      day: String(day),
      rows: rows.sort((a, b) => b.used - a.used || a.who.localeCompare(b.who)),
      silenced: rows.reduce((sum, row) => sum + row.silenced, 0),
    }
  }

  /**
   * token / 成本：按 **天 × 机器人 × 群** 聚合（R13.6），从落盘的 `usage` 记录算。
   *
   * 两个口径必须分开写清楚，否则这张表会骗人：
   *   · `runs` 是跑过的轮次，`withTokens` 是其中**provider 真报了用量**的那些；
   *     两者不等时 `tokens` 只是"已知的那部分"，所以 `unknownRuns` 要一起显示 ——
   *     与卡片 footer 同一条规矩：没上报的字段不谎报 0。
   *   · `elapsed_ms` / `steps` 不依赖 provider 上报，分母是全部轮次。
   *
  ...
  function usage(options = {}) {
    ...
    let elapsed = 0
    let steps = 0
    for (const record of store.all('usage')) {
      const day = String(record?.day ?? '')
      if (day === '' || day < cutoff) continue
      const bot = String(record.bot ?? '')
      const chat = String(record.chat ?? '')
      const key = day + '|' + bot + '|' + chat
      const bucket = buckets.get(key) ?? { day, bot, chat, runs: 0, withTokens: 0, tokens: 0, elapsed_ms: 0, steps: 0 }
      bucket.runs += 1
      runs += 1
      bucket.elapsed_ms += Number(record.elapsed_ms ?? 0)
      elapsed += Number(record.elapsed_ms ?? 0)
      bucket.steps += Number(record.steps ?? 0)
      steps += Number(record.steps ?? 0)
      const value = record.tokens === null || record.tokens === undefined ? null : Number(record.tokens)
    ...
    return {
      days,
      since: cutoff,
      total: { runs, withTokens, unknownRuns: runs - withTokens, tokens, elapsed_ms: elapsed, steps },
      byDayBotChat: [...buckets.values()].sort(
        (a, b) => String(b.day).localeCompare(String(a.day)) || a.bot.localeCompare(b.bot) || a.chat.localeCompare(b.chat),
      ),
    }
  }
  ...
  function snapshot() {
    return {
      ...
      delivery: delivery(),
      chats: chats(),
      // R13.5 / R13.6：下面两张表都是**落盘事实**，重启后不变。
      quota: quota(),
      usage: usage(),
    }
  }
