 ...
 * @property {import('./types.js').Principal[]} stand_ins `required_by` 里哪些是**代确认者**（那个域里没人，配置的 fallback 顶上）；空数组 = 都是本域的负责人
...
function gateValue(c, path, raw) {
  return /** @type {Gate} */ (
    asStrictObject(c, path, raw, [
      ...
      /*
       * 这份名单里**哪些是"代确认"**（R5.6）：那个域一个人都没有时，配置里的
       * `domainFallbacks[域]` 顶上。建任务时冻结，不随后来的配置变 ——
       * 与门禁其它快照同一个道理（"当时谁该确认"不能被事后改的配置改写）。
       * 卡片靠它把那个"代"字打出来：不写，人分不清 @chen-req 是那个域的负责人还是替身。
       */
      { name: 'stand_ins', parse: (v, p) => asArray(c, p, v, asPrincipalValue), default: () => [] },
