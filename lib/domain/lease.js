...
/**
 * **过期播报**：租约超时的第一次观察（`expired` 是"已经喊过、还在宽限期"的中间态）。
 *
 * 计数必须跟着一起走：不记次数的话每个 tick 都会再喊一遍同一件事，而"每次 tick 都喊"
 * 是让人把机器人静音的最快办法。把 +1 收进领域函数之后，调用方不再手写 `expiry_notices`。
 *
 * @param {import('./schema.js').Lease} lease
 * @returns {import('./schema.js').Lease}
 */
export function noticeLeaseExpiry(lease) {
  return { ...lease, state: 'expired', expiry_notices: Number(lease.expiry_notices ?? 0) + 1 }
}

/**
 * **收回/交回**：租约进终态 `returned`（这活不再属于原来那个人）。
 *
 * 与 `releaseLease(lease, 'expired')` 的区别在**去向**：那个是"已经过期、还在等宽限期"，
 * 这个是"已经决定不给了"。两者语义不同，所以是两个函数 —— 合成一个的话，调用方就得
 * 记住"哪种 reason 配哪种 state"，而那正是 R5.7 要消灭的写法（手写 `store.put('lease', {…})`）。
 *
 * @param {import('./schema.js').Lease} lease
 * @param {'expired'|'returned'|'dropped'|string} reason
 * @returns {import('./schema.js').Lease}
 */
export function returnLease(lease, reason) {
  return { ...lease, state: 'returned', release_reason: reason }
}
