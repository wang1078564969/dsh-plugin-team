...
export const TASK_ALLOWED = Object.freeze({
   ...
   */
  /*
   * `assigned` 也在内，但**只在没有执行者时**成立（见 `assign` 分支的守卫）：
   * `timeout_start` / `timeout_lease` / `unassign` 都把任务落成
   * `assigned + assignee:null`（"回到待派发"），而 `assign` 原来只认
   * `confirmed`/`rejected` —— 于是**每一次超时释放都会制造一个永久卡死的任务**：
   * accept/reject 因为没有 assignee 而不可能，assign/reassign/start 全是 invalid_state。
   */
  assign: Object.freeze(['confirmed', 'rejected', 'assigned']),
...
export function transitionTask(task, action, actor, ctx) {
  ...
  switch (action) {
    ...
    case 'assign': {
      ...
      /*
       * 从 `assigned` 进来只允许一种情况：**这个任务没有执行者**（被自动释放/收回过）。
       * 有执行者的任务要换人走 `reassign`（它会重置门禁与确认记录），
       * 否则"指派"就成了绕过 reassign 语义的后门。
       */
      if (task.state === 'assigned' && isAssignee(task, ctx.suggestedAssignee ?? '')) {
        return err('no_change', '这个任务已经指派给他了；换人请用 reassign')
      }
      if (task.state === 'assigned' && task.assignee !== null && !canGovern(ctx, actor)) {
        return err('forbidden', '任务已经有执行者，改派要由需求负责人或该域负责人决定')
      }
     ...
     * CI 与归档也需要身份守卫。
     *
     * `ci_start/ci_pass/ci_fail` 以前**没有任何守卫**：任何人（包括自报 actor 的面板）
     * 都能把一个任务标成"CI 通过"，而合并判定读的正是这个状态 ——
     * "人类审批 + CI 通过 + 只有 ops 能合并"三道门里的第二道因此形同虚设。
     * 推 CI 的人 = 执行者本人、需求负责人或该任务的域负责人（与 submit 同一批人）。
     */
    case 'ci_start':
      if (!isAssignee(task, actor) && !canGovern(ctx, actor) && !isDomainOwner(ctx, actor)) {
        return err('forbidden', '只有执行者本人、需求负责人或该任务所属域的负责人可以推进 CI 状态')
      }
    ...
    case 'verify': {
      ...
      if (!sat.ok) return sat
      /*
       * 硬闸 3：门禁还没全确认 —— 但**这一票要留下**。
       *
       * 之前这里直接 `err('gate_incomplete')`，于是刚记下的确认被丢掉：
       * 跨域任务要求两个人都确认时，第一个人点完还是"还差两个人"，第二个人点完
       * 还是"还差两个人" —— 永远 ping-pong（README 的已知问题，来自 hub）。
       * 正确行为是：**确认累积，缺一方不推进**（设计 04 §3.5），所以这里返回
       * `ok: true` + `partial: true`：状态不动，但门禁上多了这一票，
       * 剩下的确认人由调用方播报/催办。
       */
      if (!gateSatisfied(sat.gate, { mainOwner: ctx.requirement?.owner })) {
        const still = gatePending(sat.gate)
        const partial = withHistory(
          { ...base, gates: { ...base.gates, acceptance: sat.gate } },
          ['播报：' + actor + ' 已验收，还差 ' + still.join('、')],
        )
        return { ...partial, partial: true, pending: still }
      }
      return withHistory(
    ...
    case 'archive':
      if (!canGovern(ctx, actor)) return err('forbidden', '归档只有需求负责人或项目经理可以做')
...
function satisfyGate(task, name, actor, ctx) {
  ...
  /*
   * 名单之外，**主 owner 永远可以点头**（设计 00 §5.1："多人协作由主 owner 确认"）。
   *
   * 跨域任务的门禁列的是各域负责人；如果只认那张名单，需求负责人就被自己的需求
   * 挡在门外 —— 而他是最终对这件事负责的人。名单管的是"谁必须点头"，
   * 不是"只有谁能点头"。
   */
  const mainOwner = typeof ctx.requirement?.owner === 'string' ? ctx.requirement.owner : null
  if (listed && !gate.required_by.includes(actor) && actor !== mainOwner) {
...
export const REQ_ALLOWED = Object.freeze({
  ...
  /**
   * 需求完成：`dispatched → done`（设计 06 §3.1 + §7 完整性约束 2）。
   *
   * 这条以前**根本不存在** —— `done` 在状态枚举里，却没有任何动作能产出它，
   * 于是需求永远停在 `dispatched`，"归档"也就永远走不到。门禁是完整性约束 2：
   * 其下任务必须全部 `done`/`archived`（由调用方算好放进 `ctx.allTasksDone`）。
   */
  finish: Object.freeze(['dispatched']),
...
export function transitionRequirement(req, action, actor, ctx) {
  ...
  switch (action) {
    ...
    case 'finish': {
      if (ctx.allTasksDone !== true) {
        return err('tasks_open', '还有任务没完成，需求不能标记完成（完整性约束 2）')
      }
      if (!canGovern(ctx, actor) && !isDomainOwner(ctx, actor)) {
        return err('forbidden', '只有需求负责人或该域的负责人可以确认需求完成')
      }
      // 这里是 `ok(...)` 而不是 `withHistory(...)`：后者只定义在 `transitionTask` 内部，
      // 需求侧自己的助手叫 `ok`。写成 `withHistory` 会抛 ReferenceError，
      // 而"任务全部验收 → 需求自动收口"是这条分支唯一的出路。
      return ok({ ...req, state: 'done' }, ['播报：需求已完成（其下任务全部验收）'])
    }
...
export function availableActions(task) {
  return /** @type {TaskAction[]} */ (Object.keys(TASK_ALLOWED)).filter((a) => {
    if (!TASK_ALLOWED[a].includes(task.state)) return false
    /*
     * `assign` 在 `assigned` 上只对"没有执行者"的任务成立 —— 卡片按钮与面板按钮
     * 都读这个函数，所以这条守卫必须在这里，而不是只写在状态机里（否则界面会给出
     * 一个按下去必然被拒的按钮）。
     */
    if (a === 'assign' && task.state === 'assigned' && task.assignee !== null) return false
    return true
  })
}
