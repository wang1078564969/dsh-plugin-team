 ...
 * @property {{confirm_split?: import('./types.js').Principal[], accept?: import('./types.js').Principal[], start?: import('./types.js').Principal[], acceptance?: import('./types.js').Principal[]}} [standIns] 上面每份名单里哪些是**代确认者**（那个域没人，配置的 fallback 顶上）；缺省 = 都是本域负责人
 */
...
export function buildGates(input, specs = {}) {
  ...
  const gate = (name, required, notApplicable = false) => {
    ...
    return {
      ...
      // `standIns[name]` = 这份名单里哪些是替身（那个域一个人都没有，配置的 fallback 顶上）。
      // 卡片靠它把"代"字打出来（R5.6）；机器人承接时整份名单为空，也就没有替身。
      stand_ins: notApplicable ? [] : (Array.isArray(input.standIns?.[name]) ? input.standIns[name] : []),
      confirmed_by: [],
      due_at: null,
