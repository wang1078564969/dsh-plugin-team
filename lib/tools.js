...
// `list` 要能回答"这个 kind 认不认识"，而种类的唯一真相在 store 里。
 ...
 */
/**
 * 把 `reason` 归一成 `note`（打回/阻塞类动作两者都收）。
 *
 * 模型经常把理由放在 `reason` 里（那是最自然的名字），而 handler 读的是 `note` ——
 * 于是它明明写了原因却被回一句"要写一句原因"。在统一入口做一次归一，比让每个
 * handler 各自兼容更可靠。
 */
function normalizeArgs(name, args) {
  if (args === null || typeof args !== 'object') return args
  if (typeof args.note === 'string' && args.note !== '') return args
  if (typeof args.reason !== 'string' || args.reason === '') return args
  return { ...args, note: args.reason }
}

function actorFor(args, fallback) {
  const source = args !== null && typeof args === 'object' ? args : {}
  /*
   * 谁在做这件事：`actor` 优先，其次 `owner`（很多动作的调用方只给了负责人），
   * 最后才用调用方显式给的 fallback。
   *
   * `fallback === null` 是**"没有就返回 null"**的意思 —— 统一入口那条成员闸需要它：
   * 那里绝不能编一个默认主体出来（"缺 actor 就当成 owner 动手"正是要防的事）。
   */
  const raw = [source.actor, source.owner].find((one) => typeof one === 'string' && one !== '')
  const chosen = raw !== undefined ? raw : fallback
  if (typeof chosen !== 'string' || chosen === '') return null
  return asPrincipal(chosen)
}
...
export function createHandlers({ ctx, config, store, pool, notify, docs, recall }) {
   ...
   *
   * **每次判定现算，不用启动快照**：配置台承诺"保存即生效"，而快照会让
   * "刚把某人设成 observer"这类改动要重启才生效 —— 面板显示改了、判定还是旧的，
   * 这是最容易被当成"系统坏了"的一类不一致。成员表只有几十行，重算的代价可以忽略。
   */
  const membersNow = () =>
    resolveMembers({ members: config.memberList, domains: config.members, senders: config.feishu?.senders })
  ...
  function gateWrite(actor, needsApproval) {
    return memberCanWrite(membersNow(), { actor, action: needsApproval ?? '', needsApproval })
  }
  ...
  function tryFinishRequirement(reqId, actor) {
    ...
    /*
     * `allTasksDone` 必须显式传：状态机用它挡"还有任务没完成"（完整性约束 2）。
     * 不传的话它会在校验那一步直接返回 `tasks_open`，而这个函数拿到非 ok 就返回 null ——
     * 于是**需求永远不会自动完成，而且没有任何异常或日志**（这条曾经就是这样静默失效的）。
     */
    const result = transitionRequirement(req, 'finish', actor, { ...transitionCtx(null, req), allTasksDone: true })
   ...
   */
  /**
   * 验收人（设计 04 §3.5 跨域并行确认）：需求负责人 + 这个任务跨越的每个域的负责人。
   *
   * **域里没人时用该域的 fallback，并把它记成"代确认"**（R5.6）：卡片要能写出那个"代"字，
   * 否则人分不清 `@chen-req` 是这个域的负责人，还是替一个空域签字的替身。
   * 返回 `{ confirmers, standIns }`，由 `buildGates` 冻结进 `gate.stand_ins`。
   */
  function acceptanceConfirmers(req, domains, assignee) {
    const out = []
    const standIns = []
    const push = (who, isStandIn = false) => {
      const value = who === null || who === undefined ? '' : String(who)
      if (value === '' || value === assignee || out.includes(value)) return
      out.push(value)
      if (isStandIn) standIns.push(value)
    }
    ...
    for (const domain of domains) {
      ...
      if (owners.length === 0) push(config.domainFallbacks?.[domain] ?? null, true)
    }
    return { confirmers: out, standIns }
  }
    ...
    Promise.resolve()
      .then(() =>
        notify.notice({
          ...
          chatId,
          /*
           * id 带**小时时间片**：同一个 `card_key` 的第二次投递是 PATCH，
           * 不带时间片的话"再催一次"只会把旧卡改一下 —— 而催办的意义就是让人**再**看到。
           * 同一小时内重复扫到仍然合并（不会一小时刷屏）。
           */
          id: opts.reason + ':' + opts.task.id + ':' + new Date().toISOString().slice(0, 13),
  ...
  /**
   * 同一时刻只允许一次"驱动一轮"的任务集合。
   *
   * `run_task` 的闸门是"读任务状态 → await 起会话 → await 驱动一轮"，中间全是 await：
   * 模型并行发两个 `run_task`、面板与工具同时点、两个标签页各点一次，**两个都会过闸**，
   * 于是同一个任务驱动两轮真实模型会话（真花 token、可能真改代码），evidence 后写者赢。
   * 一个进程内的 Set 就够了：这个插件的写入者只有一个进程。
   */
  const runningTasks = new Set()

  /**
   * 需求负责人（缺省 `config.defaultOwner`）在名册里吗？
   *
   * 名册非空、而缺省负责人不在里面时，静默用一个"不在编"的人当 owner 是坏默认：
   * 门禁名单会写上他、卡片会 @ 他，而这个人可能根本不存在。这时宁可拒绝并说清怎么办。
   *
   * @returns {string|null} 拒绝理由（null = 可以用）
   */
  function ownerFallbackProblem(owner) {
    const roster = membersNow().list ?? []
    if (roster.length === 0) return null
    if (typeof owner !== 'string' || owner === '') return '需要一个负责人（owner），但配置里没有可用的默认负责人'
    if (roster.some((one) => one.key === owner)) return null
    return (
      '缺省负责人 ' + owner + ' 不在成员表里（表里有 ' + String(roster.length) + ' 个人）：' +
      '要么在调用时给出 owner，要么把他加进「成员」页，要么把配置里的 defaultOwner 改成表里的人'
    )
  }
  ...
  const actions = {
    ...
    docs(args) {
      ...
      /*
       * 草稿转正（R10.4）：机器人沉淀的知识默认落 draft，等人点头才生效。
       * 谁点的头必须写清（`actor` 走的是写动作那道闸，这里再确认一次不为空）。
       */
      if (op === 'confirm') {
        const by = actorOf(args)
        if (by === null || by === '') return { ok: false, code: 'bad_request', message: '确认文档要写清 actor：谁确认的' }
        const result = docs.confirm(String(args.id ?? ''), { by })
        if (result.ok !== true) return result
        return { ...result, what: '草稿已确认（draft → active），确认人 ' + by }
      }
      if (op !== 'write') return { ok: false, code: 'bad_request', message: 'op 是 init | list | read | write | confirm | index | stale | search' }
      const written = docs.write({
        id: String(args.id ?? ''),
    ...
    list(args) {
      ...
      /*
       * 不认识的 kind 直接回 `bad_request`：以前 `store.all('nonsense')` 会抛，
       * 而"这个 kind 不存在"是最该用一句话回答的那种输入错误。
       */
      if (KINDS[kind] === undefined) {
        return { ok: false, code: 'bad_request', message: 'kind 不认识：' + kind + '（可用：' + Object.keys(KINDS).join(' | ') + '）' }
      }
      ...
      return {
        ...
        rows: rows.map((row) => {
          /*
           * 用量与配额**没有** task/requirement 那套字段（没有 title、没有 state）：
           * 硬套成 `{state: null, title: null}` 会让人以为"对象缺字段"，
           * 所以这两种 kind 各自给自己的一份（R13.5 / R13.6）。
           */
          if (kind === 'usage') {
            return {
              id: row.id,
              fields: {
                ...
                steps: row.steps,
                // `null` = provider 没上报，不是 0（读的人不该把"不知道"当成"没花钱"）。
                tokens: row.tokens ?? null,
                token_source: row.token_source ?? null,
              },
            }
          }
          if (kind === 'quota') {
            return { id: row.id, fields: { day: row.day, bot: row.bot, who: row.who, used: row.used, silenced: row.silenced } }
          }
          return {
            id: row.id,
    ...
    create_requirement(args) {
      ...
      /*
       * 名册非空、而缺省负责人不在名册里 → 拒绝并说清怎么办。
       * 静默用一个"不在编"的人当 owner 是坏默认：门禁名单会写上他、卡片会 @ 他，
       * 而这个人可能根本不存在（出厂默认值是 `human:owner`）。
       */
      const ownerProblem = ownerFallbackProblem(owner)
      if (ownerProblem !== null) return { ok: false, code: 'owner_not_a_member', message: ownerProblem }
    ...
    propose_tasks(args) {
      ...
      for (const draft of drafts) {
         ...
         */
        const acceptance = acceptanceConfirmers(req, domains, assignee)
        const confirmers = {
          confirm_split: [req.owner],
          accept: assignee === null ? [] : [assignee],
          start: assignee === null ? [] : [assignee],
          acceptance: acceptance.confirmers,
        }
        /*
         * `initializeGates` 必须在建任务时调一次：门禁的 `due_at` 只有它和状态机的
         * `activateDue` 会填。**只靠状态机是不够的** —— 新建任务停在 `proposed`，
         * 而 `confirm_split` 门禁的激活状态就是 `proposed`，此时还没有任何跃迁发生，
         * 于是它的 `due_at` 永远是 null，调度器直接跳过（`scheduler.js:94`）：
         * "8 小时没人确认拆解就催办"这条设计承诺在实现里是装饰。
         */
        const gates = initializeGates(
          buildGates({ assignee, confirmers, standIns: { acceptance: acceptance.standIns } }, config.gates),
          'proposed',
          nowDate(),
        )
    ...
    async run_task(args) {
    /*
     * 用 try/finally 释放并发闸：无论是正常返回、超时返回还是抛异常，
     * 这个任务都不能被永久锁在 runningTasks 里（否则它再也跑不了）。
     */
    try {
        ...
        if (runningTasks.has(task.id)) {
          return {
            ...
            message: '这个任务已经有一轮在执行中（' + task.id + '）：等这一轮结束再发下一次，重复驱动会真的花两轮 token',
          }
        }
        ...
        /*
         * 用量事实**再落一条**（R13.6）：会话记录上的 `last_run` 只回答"上一轮花了多少"，
         * 而"这个机器人这个群这些天花了多少"是钱的问题。一条一轮、不聚合 ——
         * 聚合由 `lib/metrics.js` 的 `usage()` 在读的时候算。
         *
         * `tokens: null` 照实写 null（provider 没上报），不写 0：读的时候
         * `unknownRuns` 会把它们单独数出来，而不是混进总额里当"花得少"。
         */
        store.put('usage', {
          id: store.nextId('usage'),
          day: new Date().toISOString().slice(0, 10),
          bot: role === null || role === undefined ? '' : String(role),
          chat: chatIdOfTask(store, task) ?? '',
          task: task.id,
          req: task.req,
          session: run.session_id,
          elapsed_ms: Number(driven.elapsedMs ?? 0),
          steps: Number(driven.steps ?? 0),
          tokens: driven.tokens === null || driven.tokens === undefined ? null : Number(driven.tokens.value ?? 0),
          token_source: driven.tokens === null || driven.tokens === undefined ? null : String(driven.tokens.source ?? ''),
          at: new Date().toISOString(),
        })
      ...
      // 用参数里的 id 而不是函数内的 `task`：它在 try 里声明，finally 看不到它。
      runningTasks.delete(String(args?.id ?? ''))
    }
},
    ...
    verify_task(args) {
      ...
      if (committed.ok === true && result.partial !== true) {
        /*
         * 需求收口是**尽力而为但不能静默失败**：任务已经 done 了，而需求没跟上——
         * 那正是"验收完了群里什么都没发生"的来源。异常在这里被兜住，改成一句看得见的话。
         */
        try {
          const finished = tryFinishRequirement(task.req, actorFor(args, fallback))
          if (finished !== null) committed.requirement = finished
        } catch (error) {
          committed.requirement_error = String(error && error.message ? error.message : error)
          if (notify !== null && notify !== undefined) {
            Promise.resolve()
              .then(() =>
                notify.notice({
                  id: 'req-finish-failed:' + task.req,
                  chatId: chatIdOfTask(store, task),
                  title: '⚠️ 需求没能自动收口',
                  lines: ['任务 ' + task.id + ' 已验收，但需求 ' + task.req + ' 的状态推进失败了。', committed.requirement_error],
                  note: '这是一条系统报错，需要人看一眼（通常意味着需求下还有未完成任务）。',
                }),
              )
              .catch(() => {})
          }
     ...
     * 冻结之后的三个出口（设计 06 §4.1）。
     *
     * `suspended` 曾经是**唯一零出口的状态**：状态机允许 `resume`/`drop`
     * （`machine.js:203-214`），但 handlers 里没有这两个动作，而
     * `change_requirement` 会把受影响任务**全部**推进 `suspended`
     * （`tools.js` 的 change 分支）—— 于是"改一次需求"就等于把在途任务永久冻住，
     * 而且没有任何播报。面板只渲染需求动作，群动词也没有，三条路都堵死。
     */
    resume_task(args) {
      const task = loadTask(args.id)
      if (task === null) return { ok: false, code: 'not_found', message: '没有这个任务：' + String(args.id) }
      const req = loadRequirement(task.req)
      const result = transitionTask(task, 'resume', actorFor(args, req?.owner ?? config.defaultOwner), transitionCtx(task, req))
      if (result.ok !== true) return { ok: false, code: result.code, message: result.message, pending: result.pending ?? [] }
      return commit('task', result, '已解冻，任务回到它冻结前的状态')
    },

    /** 废弃一个任务（suspended/proposed/…）：它不再参与需求收口。 */
    drop_task(args) {
      const task = loadTask(args.id)
      if (task === null) return { ok: false, code: 'not_found', message: '没有这个任务：' + String(args.id) }
      const req = loadRequirement(task.req)
      const result = transitionTask(task, 'drop', actorFor(args, req?.owner ?? config.defaultOwner), transitionCtx(task, req))
      if (result.ok !== true) return { ok: false, code: result.code, message: result.message, pending: result.pending ?? [] }
      const saved = commit('task', result, '任务已废弃' + (typeof args.note === 'string' && args.note !== '' ? '：' + args.note : ''))
      /*
       * 条目**留在**需求的清单里：卡上要能看到"这条被废弃了"。
       * 收口那边由 `requirementComplete` 显式忽略 `dropped`（否则一条废弃的任务
       * 会把整个需求钉死）。
       */
      return saved
    },
    ...
    /**
     * 交回：`accepted → assigned`（执行者自己不干了，但活还在）。
     *
     * 卡片上一直有这个按钮（`broadcast.js` 的 `task.unassign`），机器动作也在，
     * 缺的是入口 —— 与"拒绝"是两件事：拒绝是"这活不该归我"，交回是"我接了但做不了"。
     */
    unassign_task(args) {
      const task = loadTask(args.id)
      if (task === null) return { ok: false, code: 'not_found', message: '没有这个任务：' + String(args.id) }
      const req = loadRequirement(task.req)
      const result = transitionTask(task, 'unassign', actorFor(args, task.assignee ?? req?.owner ?? config.defaultOwner), transitionCtx(task, req))
      if (result.ok !== true) return { ok: false, code: result.code, message: result.message, pending: result.pending ?? [] }
      const saved = commit('task', result, '已交回（任务回到待派发，等重新指派）')
      const lease = store.get('lease', task.id)
      if (lease !== null && lease.state === 'active') {
        store.put('lease', returnLease(lease, 'returned'))
      }
      return saved
    },

    /**
     * 续约：把租约往后推一个完整时限（`renewLease` 一直是零调用点）。
     *
     * 没有这个入口，**每一份租约都必然走向收回** —— "我还在做，只是慢"这件事
     * 在系统里无法表达。续约同时也是一次重新承诺，所以它推的是完整时限，
     * 不是"补到今天"。
     */
    renew_lease(args) {
      const task = loadTask(args.id)
      if (task === null) return { ok: false, code: 'not_found', message: '没有这个任务：' + String(args.id) }
      const lease = store.get('lease', task.id)
      if (lease === null) return { ok: false, code: 'not_found', message: '这个任务没有租约（还没被接受）' }
      const actor = actorFor(args, task.assignee ?? config.defaultOwner)
      const result = renewLease(lease, actor, DEFAULT_LEASE_POLICY, nowDate(), {
        allowAnyone: args.allowAnyone === true,
      })
      if (result.ok !== true) return { ok: false, code: result.reason, message: '续约被拒：' + String(result.reason) }
      const saved = store.put('lease', result.lease)
      return {
        ok: true,
        id: task.id,
        state: saved.state,
        ...
        what: '租约已续到 ' + String(saved.expires_at) + '（第 ' + String(saved.renewals) + ' 次）',
      }
    },
    ...
    reassign_task(args) {
      ...
      const lease = store.get('lease', task.id)
      if (lease !== null && lease.state === 'active') store.put('lease', releaseLease(lease, 'reassigned').lease)
      return saved
    },
    ...
    ci(args) {
      ...
      if (op === 'status') {
        const state = ciState(task, { timeoutMs, now })
        return {
          ok: true,
          id: task.id,
          state: task.state,
          ci: state,
          what: 'CI ' + state.state + (state.stuck ? '（已经等了 ' + String(Math.round(state.elapsedMs / 60000)) + ' 分钟，可能卡住了）' : ''),
        }
      }
    ...
    async repo(args) {
      ...
      if (op === 'index' || op === 'overview') {
        ...
        if (op === 'index') {
          return {
            ok: true,
            repo: repoName,
            what: '扫了一遍 ' + repoName + ' 的检出（只读，没有落盘）',
            root,
            dirs: index.dirs,
      ...
      if (op === 'approve') {
        ...
        const member = (membersNow().list ?? []).find((one) => one.key === actor) ?? null
      ...
      if (op === 'merge') {
        ...
        const decision = mergeDecision(task, {
          ...
          actorCanApprove: (membersNow().list ?? []).find((one) => one.key === actor)?.canApprove ?? [],
          now: nowDate(),
        })
        if (decision.ok !== true) {
    ...
    tick(args) {
      ...
      for (const item of due.gates) {
        ...
        if (result.ok !== true) {
          /*
           * 到期门禁撞上一个"当前状态不允许"的跃迁：这条门禁**已经过期了**。
           * 不作废的话，`scanDue` 每轮都会把它再捞出来，于是每分钟重试一次同一个
           * 非法跃迁、只往 `decided` 里塞一行 —— 日志刷屏而群里零动静
           * （自动释放之后就是这种情况：任务已经回到待派发，`timeout_start` 不再适用）。
           */
          if (result.code === 'invalid_state' || result.code === 'not_applicable') {
            const gates = item.task.gates ?? {}
            const gate = gates[item.gate]
            if (gate !== undefined && gate !== null) {
              writeTask({
                ...item.task,
                gates: { ...gates, [item.gate]: { ...gate, due_at: null, cancelled_at: now.toISOString(), cancelled_reason: result.code } },
              })
              decided.push({ task: item.task.id, gate: item.gate, action, ok: false, code: result.code, gate_cancelled: true, notified: false })
              continue
            }
          }
      ...
      for (const item of due.leases) {
        ...
        const saved = writeTask(result.next)
        store.put('lease', returnLease(lease, 'expired'))
  ...
  /*
   * 把成员闸挂在**统一入口**上，而不是逐个 handler 手写。
   *
   * 为什么必须这样：闸原来只写在 4 个 handler 里（accept/start/submit/verify），
   * 于是 `role: "observer"` 的人照样能指派、拒绝、阻塞、归档、废弃、推 CI ——
   * "观察者只读"这句话在代码里没有落点，而漏掉一个 handler 是**静默**的。
   * 挂在这里，新增动作默认就在闸内。
   *
   * 两种情况不需要 actor：纯读动作，以及按 `op` 判定的读子命令
   * （`docs op=list/read/search/stale`、`repo op=branch/index`）。
   */
  const ACTOR_FREE = new Set(['list', 'show', 'list_chats', 'recall', 'tick'])
  const ACTOR_FREE_OPS = {
    docs: new Set(['list', 'read', 'search', 'stale']),
    repo: new Set(['branch', 'index']),
  }
  function needsActor(name, args) {
    if (ACTOR_FREE.has(name)) return false
    const free = ACTOR_FREE_OPS[name]
    if (free === undefined) return true
    const fallbackOp = name === 'docs' ? 'list' : 'branch'
    const op = typeof args?.op === 'string' && args.op !== '' ? args.op : fallbackOp
    return !free.has(op)
  }
  ...
  for (const [name, handler] of Object.entries(actions)) {
    /*
     * ⚠️ 包装函数**必须保持同步语义**：很多调用点（`api.js` 的 snapshot、
     * 群命令、测试）把 handler 当普通函数用，异步化的那一版会让
     * `handlers.list()` 变成 Promise，而调用方读的是 `.rows`。
     * 闸本身是同步的（成员表在内存里），所以不需要 async。
     */
    wrapped[name] = (rawArgs) => {
      const args = normalizeArgs(name, rawArgs)
      if (typeof handler !== 'function' || needsActor(name, args) !== true) return handler(args)
      /*
       * 没给 actor 时归到**机器人侧**（`bot:team`），不是归到默认负责人。
       *
       * 两个理由：
       *   1. 冒充一个人是可观测性上最坏的选择 —— 审计里会写成"某某人干的"，
       *      而实际上没人干，那是模型/定时器在动；
       *   2. 名册一旦建立就是白名单，而 `config.defaultOwner` 的出厂值是
       *      `human:owner`（几乎不在名册里）→ 把兜底落到它身上，会让
       *      "模型自己跑一轮任务"这种正常路径整个被拒。
       * 机器人侧的权限由 `botCan` / 作用域闸判，不归成员表管。
       */
      const actor = actorFor(args, null) ?? asPrincipal('bot:team')
      const gate = gateWrite(actor, name === 'verify_task' ? 'acceptance' : null)
      if (gate.ok !== true) return { ok: false, code: gate.code ?? 'member_readonly', message: gate.message }
      return handler(args)
    }
  }
  return wrapped
...
export function buildTeamTool(handlers, options = {}) {
  ...
  return {
    ...
    description:
      ...
      'resume_task/drop_task 解冻与废弃（需求变更会把任务冻结，这两个是出口）；unassign_task 交回；renew_lease 续约；' +
      '需求治理：finish_requirement / archive_requirement / change_requirement（变更后要 reconfirm_requirement）/ ' +
      'suspend_requirement / resume_requirement / drop_requirement / block_requirement / unblock_requirement；' +
      '任务治理：reassign_task 转派 / unassign_task 交回 / resume_task 解冻 / drop_task 废弃 / renew_lease 续约；' +
      '群与身份：set_primary_bot 改群主机器人 / list_chats 看群；' +
    ...
    parameters: {
      ...
      properties: {
        ...
        /*
         * `note` 的说明必须按 handler 的**真实读法**写：它是 `block_task` /
         * `change_requirement` / `drop_requirement` / `block_requirement` 的**必填原因**
         * （缺了就 `bad_request`），只有 `run_task` 把它当补充说明。
         * 以前这里只写"run_task 的补充说明"，模型据此漏传 → 得到一句
         * "阻塞要写一句原因"，而它以为自己没漏。
         */
        note: {
          type: 'string',
          description:
            '原因或补充说明。**必填**：block_task / block_requirement / change_requirement / drop_requirement（写清为什么）；' +
            '可选：run_task 给执行会话的补充说明、reject_review 打回理由、repo op=approve 的批注',
        },
        reason: { type: 'string', description: 'note 的别名（打回/阻塞类动作两者都可以）' },
        ...
        op: {
          type: 'string',
          description: '子动作：docs 用 init|list|read|write|confirm|search|index|stale；ci 用 start|pass|fail|status；repo 用 branch|index|overview|link_mr|approve|merge',
        },
        ...
        actor: { type: 'string', description: '谁在做这件事（写操作必须有）' },
        /*
         * 下面这些 handler 真的会读，但 schema 里没有声明 —— 而工具 schema 是
         * `additionalProperties: false`，于是它们**从工具调用根本传不进来**：
         * `docs op=write status=draft`、`record_decision status=proposed`、
         * `repo op=branch slug=…`、`repo op=merge requiredApprovals=2` 全都做不到。
         */
        slug: { type: 'string', description: '分支名的英文后缀（repo op=branch）' },
        requiredApprovals: { type: 'number', description: '合并前至少几个审批（repo op=merge，缺省读配置）' },
        status: { type: 'string', description: 'docs op=write 的文档状态 / record_decision 的决策状态（proposed 等）' },
        source: { type: 'string', description: '决策来源：feishu_chat | dsh_session | doc | git_mr | manual' },
        visibility: { type: 'string', description: '文档可见性：team | project:<名字> | private' },
        excerpts: { type: 'array', items: { type: 'string' }, description: 'create_requirement 的来源摘录（群消息原文）' },
        message_id: { type: 'string', description: 'create_requirement 的来源消息 id' },
        chat_id: { type: 'string', description: 'create_requirement 的来源群 id' },
        approved: { type: 'boolean', description: 'repo op=approve：true 批准（缺省）/ false 驳回' },
        allowAnyone: { type: 'boolean', description: 'renew_lease：允许非持有人续约（维护用）' },
        staleAfterDays: { type: 'number', description: 'docs op=stale 的"多久没更新算过期"阈值（默认 180 天）' },
        query: { type: 'string', description: 'recall / docs op=search 的查询词' },
