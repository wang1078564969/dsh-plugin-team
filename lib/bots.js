 ...
 *          defaultAppId?: string, boundBotId?: string|null, mentionedAppIds?: string[]}} input
...
export function routeBots(input) {
  ...
  /*
   * 这条消息**按 open_id 点名了哪几个应用**（调用方拿 `mentionedOpenIds` 与每个应用
   * 自己的 botOpenId 比对得出，见 lib/team.js 的 `mentionedAppsOf`）。
   *
   * 为什么它必须能推翻 `appId` 那道过滤：一个群里两台机器人时，同一个事件在两条连接上
   * 各到一次，去重只放**先到的那一条**过去 —— 于是"谁收到了"是竞态，而"谁被点名"是事实。
   * 只按收到的那条连接过滤，会得到"@ 了 B，A 的连接先到 → 候选里没有 B → 没人回答"，
   * 而且时灵时不灵（2026-09-12 用户就是这么中招的）。
   */
  const mentioned = Array.isArray(input.mentionedAppIds) ? input.mentionedAppIds.filter((one) => typeof one === 'string' && one !== '') : []
  const enabled = orderedBots((input.bots ?? []).filter((one) => one.enabled === true))
  const appOf = (bot) => (bot.feishu.appId !== '' ? bot.feishu.appId : defaultAppId)

  const serving = enabled.filter((bot) => {
    const botApp = appOf(bot)
    const itsAppWasMentioned = botApp !== '' && mentioned.includes(botApp)
    if (!itsAppWasMentioned && appId !== '' && botApp !== appId) return false
  ...
  // 1) 被点名的那台：最硬的信号（按身份，不按名字），先于任何文本匹配。
  for (const bot of serving) if (mentioned.includes(appOf(bot))) ranked.push(bot)
  // 2) 正文里写了它的名字（名字和飞书里的名字不一致时的兜底）。
  if (text !== '') {
    for (const bot of serving) if (!ranked.includes(bot) && addressesBot(text, bot)) ranked.push(bot)
  }
  // 3) 这个群已经绑定的主。
...
export function pickPrimaryBot(config, input) {
  ...
  const candidates = routeBots({
    ...
    /* 被点名的身份（open_id → 应用）也要一路传下去：定归属用的是同一套路由规则。 */
    mentionedAppIds: Array.isArray(input?.mentionedAppIds) ? input.mentionedAppIds : [],
    boundBotId: bound,
  })
 ...
 * 这个群在册吗（准入的判据）。
 *
 * `allowlist` 与机器人自己的 `chats` 是**并集**（写白名单不等于"只服务这些群"：
 * 机器人登记过的群照样在范围内）。判据只有一条：这个群有没有归属。
 */
export function isRegisteredChat(config, chatId) {
  const id = String(chatId ?? '')
  if (id === '') return { ok: false, reason: 'no-chat-id' }
 ...
 * 准入要不要拦？
 *
 * - `feishu.requireRegisteredChat: true` → 拦；
 * - **写了 `feishu.chatAllowlist` 也拦**（哪怕没打开 requireRegisteredChat）：
 *   写一张白名单就是在表达"只在这些群里工作"，而以前它只在开关打开时才被读，
 *   于是"我只配了白名单"这件事**完全没效果** —— 一个看起来生效、实际不生效的旋钮，
 *   比没有这个旋钮更坏；
 * - 两个都没有 → 不拦（默认：不收紧，但机器人自己的 `feishu.chats` 仍然决定谁回答）。
 */
export function admissionRequired(config) {
  const allow = Array.isArray(config?.feishu?.chatAllowlist) ? config.feishu.chatAllowlist : []
  return allow.length > 0 || config?.feishu?.requireRegisteredChat === true
}
