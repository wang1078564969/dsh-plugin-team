  ...
  factory: function (require) {
    ...
    function TeamLogsPage(props) {
      ...
      if (delivery !== null) {
         ...
         */
        var fallbackRate = asNumber(delivery.fallbackRate, 0)
        var sentTotal = asNumber(delivery.total, 0)
        var deliveryNodes = []
        deliveryNodes.push(React.createElement('div', { className: 'teamled-row', key: 'h' },
          ...
          React.createElement('span', {
            ...
            key: 'rate',
          }, sentTotal === 0 ? '降级率 —（还没有成功投递）' : '降级率 ' + (fallbackRate * 100).toFixed(1) + '%'),
        ))
        var tiers = asArray(delivery.tiers)
        if (tiers.length > 0) {
          var tierRows = []
        ...
        deliveryNodes.push(React.createElement('div', { className: 'teamled-note', key: 'n' },
          '发出条数 = 新卡 + 原地更新；「降级」= 卡没送出去、落到纯文本/文本加附件（' +
          FALLBACK_VIAS.join(' / ') + '）。降级率高说明卡片渲染有问题，不是消息发不出去。'))
