...
/** 日志是**滚动**的东西：页面每 3 秒刷它，而不是把整份配置快照重算一遍。 */
const LOGS_URL = '/api/team/logs'
/**
 * 宿主半边的**自述**路由：这次加载成没成、失败在哪一步。
 *
 * 它由 `lib/index.js`（entry）自己挂，**不依赖 `lib/team.js` 是否活着** —— 正是因为
 * 台账路由 404 时它还得在，才需要它：那种情况下唯一说得清原因的接口就是这一条。
 * 与 host 的路径必须一致，`test/client-half.test.mjs` 逐字比对两边。
 */
const BOOT_URL = '/api/team/boot'
/** 自动刷新间隔（设计 05 §4.0：默认 3 秒，可关）。 */
const LOGS_REFRESH_MS = 3000
/**
 * 五级降级链里的后两级 = 降级（`lib/feishu/cards.js` 的 `FALLBACK_VIAS`）。
 *
 * 页面是经典脚本，import 不到 host 的常量，所以这里必然是一份副本 ——
 * 于是 `test/client-half.test.mjs` 有一条断言逐字比对两边，改了一边忘了另一边
 * 会在测试里红，而不是在群里红。
 */
const FALLBACK_VIAS = ['text', 'text-with-file']
 ...
 */
/*
 * 域清单不再由面板硬编码：host 在 `config.knownDomains` 里给出（设计 10 个，
 * 也允许安装自己加）。硬编码一份的后果是"配置里能写、面板上编不了"。
 */
const FALLBACK_MEMBER_DOMAINS = ['pm', 'product', 'design', 'requirement', 'development', 'testing', 'ops', 'data', 'security', 'docs']
 ...
 * 需求侧的动作 → 工具动作名。
 *
 * 需求以前在面板上是**只读**的：状态机里 confirm/confirm_split/archive/suspend…
 * 全都有，面板一个按钮都没有，于是"这个需求做完了"只能去命令行敲工具。
 * `change` 与 `drop` 故意不在表里：它们要求写原因，而这一页没有理由输入框，
 * 与其静默丢掉理由，不如把它们留给 `team` 工具（卡片上会写一句提示）。
 */
const REQ_ACTION_CALL = {
  confirm: 'confirm_requirement',
  confirm_split: 'confirm_split',
  finish: 'finish_requirement',
  archive: 'archive_requirement',
  reconfirm: 'reconfirm_requirement',
  suspend: 'suspend_requirement',
  resume: 'resume_requirement',
  block: 'block_requirement',
  unblock: 'unblock_requirement',
}
const REQ_ACTION_ORDER = ['confirm', 'confirm_split', 'finish', 'archive', 'reconfirm', 'suspend', 'resume', 'block', 'unblock']
const REQ_ACTION_LABEL = {
  confirm: '确认需求',
  confirm_split: '确认拆解',
  finish: '标记完成',
  archive: '归档',
...
const CSS = [
  ...
  '.teamlog-list { max-height: 420px; overflow: auto; font-size: 12px; line-height: 1.5; }',
  '.teamlog-row { display: flex; gap: 8px; padding: 2px 0; border-bottom: 1px solid var(--dsw-alias-border-l1); }',
  '.teamlog-at { color: var(--dsw-alias-label-secondary); white-space: nowrap; }',
  '.teamlog-level { width: 44px; text-transform: uppercase; }',
  '.teamlog-source { width: 88px; color: var(--dsw-alias-label-secondary); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }',
  '.teamlog-msg { flex: 1; word-break: break-word; }',
  '.teamlog-warn .teamlog-level { color: var(--dsw-alias-label-warning, var(--dsw-alias-label-primary)); }',
  '.teamlog-error .teamlog-level { color: var(--dsw-alias-label-danger, var(--dsw-alias-label-primary)); font-weight: 600; }',
...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    /**
     * 台账路由 404 时，去问 entry「你自己起没起来」。
     *
     * 两种 404 长得一模一样，原因却完全不同：插件行没挂载（这一层就该说"没挂"），
     * 和**插件行挂上了、宿主半边激活失败**。后者最骗人 —— 浏览器半边是静态脚本，
     * 面板照常渲染、页签都在，接口却一个都不存在；2026-09-12 的事故就是这一种，
     * 当时唯一的证据在 `load-report.txt` 里，而没人知道要去看它。
     *
     * 探不到（旧版本插件、或 connection 服务不可用，它自己也 404）或它说一切正常时
     * 返回 null：**不编原因**，让调用点保留原来那句保守的说明。
     */
    function bootExplanation() {
      return callApiTo(BOOT_URL, 'GET', undefined, GET_TIMEOUT_MS).then(function (outcome) {
        var payload = asObject(outcome.payload)
        if (payload === null || payload.ok === true) return null
        var phase = asText(payload.phase)
        var message = nonEmptyString(payload.message) ? payload.message : '没有留下原因'
        var hint = nonEmptyString(payload.hint) ? payload.hint : ''
        return (
          '宿主半边没能起来（' + (phase === '' ? '步骤未知' : phase) + '）：' + message +
          (hint === '' ? '' : '。' + hint)
        )
      })
    }
    ...
    function buildConfigForm(config) {
      ...
      var fields = {
        ...
        'feishu.requireRegisteredChat': feishu.requireRegisteredChat === true,
        'feishu.chatAllowlist': joinList(feishu.chatAllowlist),
      ...
      var i
      var memberDomains = readTextList(cfg.knownDomains)
      if (memberDomains.length === 0) memberDomains = FALLBACK_MEMBER_DOMAINS
      for (i = 0; i < memberDomains.length; i += 1) {
        fields['domains.' + memberDomains[i]] = joinList(domains[memberDomains[i]])
      }
    ...
    function buildConfigPatch(original, fields, rows, editable) {
      ...
      if (can('domains')) {
        var domains = asObject(cfg.domains) === null ? {} : cfg.domains
        var submitDomains = readTextList(cfg.knownDomains)
        if (submitDomains.length === 0) submitDomains = FALLBACK_MEMBER_DOMAINS
        var nextDomains = {}
        var domainsChanged = false
        for (var m = 0; m < submitDomains.length; m += 1) {
          var domain = submitDomains[m]
       ...
       */
      if (can('feishu.chatAllowlist')) {
        var allowList = parseList(form['feishu.chatAllowlist'])
        if (sameList(allowList, readTextList(feishu.chatAllowlist)) !== true) { feishuPatch.chatAllowlist = allowList; feishuChanged = true }
      }
      var flags = ['requireMention', 'respond', 'buttons', 'addressedOverridesIntent', 'requireRegisteredChat']
    ...
    function TeamConfigPage(props) {
      ...
      /*
       * 域清单：host 的 `config.knownDomains`（设计 10 个）优先，拿不到才用本地兜底。
       * 面板不再自己维护一份 —— "配置里能写、面板上编不了"就是这么来的。
       */
      var memberDomains = readTextList(
        payload === null || asObject(payload.config) === null ? null : asObject(payload.config).knownDomains,
      )
      if (memberDomains.length === 0) memberDomains = FALLBACK_MEMBER_DOMAINS
      ...
      /*
       * "改了没反应"最容易发生在配置来源被 row config 固定时（来源顺序：
       * 环境变量 > row config > config.json > 默认值）。与其让人自己猜，不如直接列出来。
       */
      var pinnedKeys = readTextList(props.payload === null || props.payload === undefined ? null : props.payload.rowPinned)
      if (pinnedKeys.length > 0) {
        children.push(React.createElement('div', { className: 'teamled-panel teamled-panel-warn', key: 'pinned' },
          React.createElement('div', { className: 'teamled-msg teamled-warn', key: 'm' },
            '这些键被 profile 的 row config 固定住了，面板改了不会生效：' + pinnedKeys.join('、')),
          React.createElement('div', { className: 'teamled-note', key: 'n' },
            '要让面板说了算，就把它们从 profile 的 cordis.patch.yml 里删掉。'),
        ))
      }
      ...
      /*
       * 可选服务缺失是静默降级：没有 timer 就没有门禁超时扫描 / 租约回收 / 日报 / 补发；
       * 没有 agents 就跑不了任务。这两件事必须显式说出来，否则使用者只会看到"任务怎么不动"。
       */
      var caps = asObject(props.payload === null || props.payload === undefined ? null : props.payload.capabilities)
      if (caps !== null) {
        var missing = []
        if (caps.timer !== true) missing.push('timer（门禁超时扫描 / 租约回收 / 日报 / 节流补发都不会发生；可用「扫超时」手动兜底）')
        if (caps.agents !== true) missing.push('agents（run_task 起不了执行会话）')
        if (caps.connection !== true) missing.push('connection（本页的数据来自它）')
        if (missing.length > 0) {
          children.push(React.createElement('div', { className: 'teamled-panel teamled-panel-warn', key: 'caps' },
            React.createElement('div', { className: 'teamled-msg teamled-warn', key: 'm' },
              '这个 profile 缺少可选服务：' + missing.join('；')),
            React.createElement('div', { className: 'teamled-note', key: 'n' },
              '这些能力缺失时插件会降级继续跑，但"到点该发生的事"不会发生 —— 上面那条说明是它的后果。'),
          ))
        }
      }
      ...
      if (payload !== null && editableKeys.length === 0) {
        ...
        if (can('domains')) {
          var domainNodes = []
          for (var mi = 0; mi < memberDomains.length; mi += 1) {
            domainNodes.push(field('f-d-' + memberDomains[mi], memberDomains[mi],
              textInput('domains.' + memberDomains[mi], 'i', 'human:<名字>, bot:<角色>'),
              mi === 0 ? '逗号 / 顿号分隔；空着就是这个域没人（没配的域也照样显示）。成员的 openId 在「成员」页绑定。' : ''))
          }
          formSections.push(section('f-domains', '成员域（domains）', '每个域一行，逗号分隔（清单来自 host 的 config.knownDomains）', domainNodes))
        }
        ...
        if (can('feishu.requireRegisteredChat')) {
          feishuNodes.push(field('f-f-regchat', 'feishu.requireRegisteredChat',
            checkInput('feishu.requireRegisteredChat', 'c'),
            '准入第一道：只处理**登记过的群**（某个启用机器人的 chats 里列了它，或在 chatAllowlist 里）。默认关：被拉进哪个群就在哪个群干活'))
        }
        if (can('feishu.chatAllowlist')) {
          feishuNodes.push(field('f-f-allow', 'feishu.chatAllowlist', textInput('feishu.chatAllowlist', 'i', 'oc_…, oc_…'),
            '明确放行的群（与机器人自己的 chats 取并集）；只在上面那个开关打开时有意义'))
        }
        /*
         * 任务类型 → 所需域：**只读**展示。
         * 它决定"谁会进确认名单"，所以要看得到；但逐个类型的编辑属于配置文件那层
         * （面板给一个只读表 + 一句"改这个键"的指引，比给九个 JSON 输入框诚实）。
         */
        var taskTypeNodes = []
        var taskTypes = asObject(payload === null || asObject(payload.config) === null ? null : asObject(payload.config).taskTypes)
        if (taskTypes !== null) {
          var typeNames = Object.keys(taskTypes)
          for (var ti = 0; ti < typeNames.length; ti += 1) {
            var spec = asObject(taskTypes[typeNames[ti]])
            if (spec === null) continue
            taskTypeNodes.push(line('f-tt-' + typeNames[ti], 'teamcfg-hint',
              typeNames[ti] + '（' + asText(spec.label) + '）→ ' + readTextList(spec.domains).join(' + ') +
                (nonEmptyString(spec.touches) ? ' · ' + asText(spec.touches) : '')))
          }
        }
        if (taskTypeNodes.length > 0) {
          formSections.push(section('f-tasktypes', '任务类型 → 所需域（只读）',
            '决定一个任务跨越哪些域 —— 也就是谁会被拉进确认名单。改它用配置文件的 taskTypes 键',
            taskTypeNodes))
        }
        var flagNodes = []
    ...
    function TeamSessionsPage(props) {
      ...
      children.push(React.createElement('div', { className: 'teamled-panel', key: 'head' },
        ...
        React.createElement('div', { className: 'teamled-note', key: 'n' },
          '来源：GET ' + CONFIG_URL + ' 的 sessions。**每个机器人 × 每个群一条会话**（team-bot-<机器人>-<群>）：同一个群里两台机器人各有各的上下文与记忆，不是一个群共用一个。登记发生在这个机器人第一次被看见在这个群里时（收到它应用的消息、或被 @）；真正能跑的会话在它**第一次回答**时才建立，那时它也会出现在 GUI 侧栏的工作区下面。'),
      ))
      ...
      for (var g = 0; g < groups.length; g += 1) {
        ...
        for (var s = 0; s < current.items.length; s += 1) {
          ...
          var head = [
            ...
            /*
             * 轮次 0 多半不是"这个群没人说话"，而是"**这台机器人**还没在这个群说过话" ——
             * 群里每台机器人各有一条自己的会话（机器人 × 群），登记在它第一次被看见在这个群里
             * （收到它的消息，或被 @）的时候，真正的 DSH 会话在它第一次回答时才打开。
             * 不写这一句，屏幕上就是"群里明明有消息，会话却 0 轮"，看着像坏了。
             */
            asNumber(session.turns, 0) === 0
              ? React.createElement('span', { className: 'teamled-tag', key: 'new' }, '还没在这个群说过话')
              : null,
          ]
    ...
    /**
     * 「日志」页：**出问题时人第一反应是"刚才发生什么了"**（设计 05 §4.0）。
     *
     * 三块内容，各回答一个问题：
     *   1. 日志本体：按级别/来源筛，可读**重启前**的那些（`file=1`）；
     *   2. 收件箱：漏单数 + 每条消息的**分诊结论**（"为什么没建单"）；
     *   3. 投递统计：降级率（哪一级在发）与机器人的发言占比。
     */
    function TeamLogsPage(props) {
      var payload = asObject(props.payload)
      var children = []
      /*
       * 失败要有自己的第一屏。**第一次读就失败**时 `payload` 永远是 null，
       * 如果先判 loading，页面就会一直显示"读取日志中…" —— 人看到的是一张
       * 永远转不完的页，而真正的原因（路由不存在 / 500）一个字都不显示。
       * 这也正是这台机器上被报过的那个症状，所以失败必须排在加载前面。
       */
      function logsErrorPanel(key) {
        return React.createElement('div', { className: 'teamled-panel teamled-panel-err', key },
          React.createElement('div', { className: 'teamled-msg teamled-err', key: 'm' }, '读取日志失败：' + asText(props.error)),
          React.createElement('div', { className: 'teamled-note', key: 'n' }, '接口：' + LOGS_URL + '（GET）。插件行没挂载或 connection 不可用时这条路由就不存在。'),
          React.createElement('div', { className: 'teamled-note', key: 'n2' }, '点下面的「刷新」再试一次：这一页不会因为一次失败就停在加载态。'),
        )
      }
      if (payload === null && nonEmptyString(props.error)) {
        children.push(logsErrorPanel('err'))
        // 还是给一个刷新入口：没有它，一次失败就只剩手动重开页面。
        children.push(React.createElement('div', { className: 'teamled-panel', key: 'retry' },
          React.createElement('button', {
            className: 'teamled-btn',
            type: 'button',
            title: '重新读取日志（GET ' + LOGS_URL + '）',
            disabled: props.fetching === true,
            onClick: function () { props.onReload() },
          }, props.fetching === true ? '刷新中…' : '刷新'),
        ))
        return React.createElement('div', { className: 'teamroster', key: 'logs' }, children)
      }
      if (payload === null) {
        children.push(React.createElement('div', { className: 'teamled-panel', key: 'loading' },
          React.createElement('div', { className: 'teamled-msg teamled-info' }, '读取日志中…'),
          React.createElement('div', { className: 'teamled-note' }, 'GET ' + LOGS_URL + '。这一页每 ' +
            String(Math.round(LOGS_REFRESH_MS / 1000)) + ' 秒自动刷一次（可以关）。'),
        ))
        return React.createElement('div', { className: 'teamroster', key: 'logs' }, children)
      }

      if (nonEmptyString(props.error)) {
        children.push(logsErrorPanel('err'))
      }
      var log = asObject(payload.log) === null ? {} : payload.log
      var rows = asArray(log.rows)
      var counts = asObject(log.counts) === null ? {} : log.counts
      var sources = readTextList(log.sources)
      var messages = asObject(payload.messages) === null ? {} : payload.messages
      var delivery = asObject(payload.delivery)

      /* ---- 头部：刷新与筛选 ---- */
      var levelNodes = []
      var levels = ['debug', 'info', 'warn', 'error']
      for (var li = 0; li < levels.length; li += 1) {
        var levelName = levels[li]
        levelNodes.push(React.createElement('button', {
          className: 'teamled-btn' + (props.level === levelName ? ' teamled-btn-primary' : ''),
          type: 'button',
          key: 'lv-' + levelName,
          title: '只看 ' + levelName + ' 及以上',
          onClick: function (name) { return function () { props.onLevel(name) } }(levelName),
        }, levelName + '（' + String(asNumber(counts[levelName], 0)) + '）'))
      }
      var sourceOptions = [React.createElement('option', { value: '', key: 'all' }, '全部来源')]
      for (var si = 0; si < sources.length; si += 1) {
        sourceOptions.push(React.createElement('option', { value: sources[si], key: 's' + String(si) }, sources[si]))
      }
      children.push(React.createElement('div', { className: 'teamled-panel', key: 'head' },
        React.createElement('div', { className: 'teamled-row', key: 'h' },
          React.createElement('span', { className: 'teamled-panel-title', key: 't' }, '日志（' + String(rows.length) + '）'),
          React.createElement('span', { className: 'teamcfg-spacer', key: 'sp' }),
          React.createElement('label', { className: 'teamcfg-check', key: 'auto' },
            React.createElement('input', {
              type: 'checkbox',
              checked: props.autoRefresh === true,
              onChange: function (event) { props.onAuto(event.target.checked) },
            }),
            ' 每 ' + String(Math.round(LOGS_REFRESH_MS / 1000)) + ' 秒自动刷新'),
          React.createElement('button', {
            className: 'teamled-btn',
            type: 'button',
            key: 'reload',
            /* 页面上不止一颗「刷新」（台账页也有一颗）：给它一个能对上的名字。 */
            title: '重新读取日志（GET ' + LOGS_URL + '）',
            disabled: props.fetching === true,
            onClick: function () { props.onReload() },
          }, props.fetching === true ? '刷新中…' : '刷新'),
        ),
        React.createElement('div', { className: 'teamled-row', key: 'filters' }, levelNodes),
        React.createElement('div', { className: 'teamled-row', key: 'src' },
          React.createElement('select', {
            className: 'teamcfg-select',
            value: props.source,
            onChange: function (event) { props.onSource(event.target.value) },
          }, sourceOptions),
          React.createElement('span', { className: 'teamled-note' },
            '文件：' + (nonEmptyString(log.file) ? asText(log.file) : '（接口没给）') +
            '（读到了 ' + String(asNumber(log.fromFile, 0)) + ' 条重启前的）'),
        ),
        React.createElement('div', { className: 'teamled-note', key: 'n' },
          '来源：GET ' + LOGS_URL + '。写日志时**先脱敏再落盘**：密钥一类的值不会进文件，也不会进这个页面。'),
      ))

      /* ---- 日志行 ---- */
      if (rows.length === 0) {
        children.push(React.createElement('div', { className: 'teamled-panel', key: 'empty' },
          React.createElement('div', { className: 'teamled-note', key: 'n' }, '这个筛选下没有日志。放低级到 debug、或把来源切回"全部来源"。'),
        ))
      } else {
        var logNodes = []
        for (var i = rows.length - 1; i >= 0; i -= 1) {
          ...
          if (row === null) continue
          logNodes.push(React.createElement('div', { className: 'teamlog-row teamlog-' + asText(row.level), key: 'r' + String(i) },
            React.createElement('span', { className: 'teamlog-at' }, stamp(row.at)),
            React.createElement('span', { className: 'teamlog-level' }, asText(row.level)),
            React.createElement('span', { className: 'teamlog-source' }, asText(row.source)),
            React.createElement('span', { className: 'teamlog-msg' }, asText(row.message)),
          ))
        }
        children.push(React.createElement('div', { className: 'teamled-panel', key: 'rows' },
          React.createElement('div', { className: 'teamlog-list', key: 'l' }, logNodes),
        ))
      }

      /* ---- 收件箱：漏单与分诊结论 ---- */
      var msgRows = asArray(messages.rows)
      var unconsumed = asNumber(messages.unconsumed, 0)
      var inboxNodes = []
      for (var mi = msgRows.length - 1; mi >= 0 && inboxNodes.length < 40; mi -= 1) {
        var msg = asObject(msgRows[mi])
        if (msg === null) continue
        var verdict = asText(msg.triageKind) === '' ? '（没有分诊结论）' : asText(msg.triageKind)
        var why = asText(msg.ignoredReason)
        /*
         * 带**原文预览**：排障时第一个问题是"这条到底是哪句话"，
         * 只有 chat_id + 结论的话，人还得自己去翻群。
         */
        var preview = asText(msg.preview)
        inboxNodes.push(React.createElement('div', { className: 'teamlog-row', key: 'm' + String(mi) },
          React.createElement('span', { className: 'teamlog-at' }, stamp(msg.at)),
          React.createElement('span', { className: 'teamlog-source' }, asText(msg.chatId)),
          React.createElement('span', { className: 'teamlog-msg' },
            (preview === '' ? '（没有正文：图片/文件或空消息）' : preview) + ' → ' +
              verdict + (why === '' ? '' : ' · ' + why) + (asText(msg.duplicateOf) === '' ? '' : ' · 并入 ' + asText(msg.duplicateOf)) +
              (asText(msg.recordedBy) === '' ? '' : ' · 记在 ' + asText(msg.recordedBy))),
        ))
      }
      children.push(React.createElement('div', { className: 'teamled-panel', key: 'inbox' },
        React.createElement('div', { className: 'teamled-row', key: 'h' },
          React.createElement('span', { className: 'teamled-panel-title', key: 't' }, '收件箱（' + String(asNumber(messages.total, 0)) + '）'),
          React.createElement('span', { className: 'teamcfg-spacer', key: 'sp' }),
          React.createElement('span', { className: unconsumed > 0 ? 'teamled-tag teamled-tag-err' : 'teamled-tag', key: 'u' },
            '漏单 ' + String(unconsumed)),
        ),
        React.createElement('div', { className: 'teamled-note', key: 'n' },
          '「漏单」= 看了、什么都没产出、也没写下原因的消息（设计 04 §11 第三条）。下面每条都带**分诊结论**：' +
          '"为什么没建单"要能从这一屏回答，而不是去读代码。'),
        React.createElement('div', { className: 'teamlog-list', key: 'l' }, inboxNodes),
      ))

      /* ---- 入站资产：图片/文件的 `asset://` 引用（设计 04 §9） ---- */
      var assetsPayload = asObject(payload.assets)
      if (assetsPayload !== null) {
        var assetRows = asArray(assetsPayload.rows)
        var assetNodes = []
        assetNodes.push(React.createElement('div', { className: 'teamled-row', key: 'h' },
          React.createElement('span', { className: 'teamled-panel-title', key: 't' }, '入站资产（' + String(asNumber(assetsPayload.count, 0)) + '）'),
        ))
        if (assetRows.length === 0) {
          assetNodes.push(React.createElement('div', { className: 'teamled-note', key: 'e' },
            '还没有收到过图片或文件。群里发一张图，它会落到 <dataDir>/assets/ 下，并得到一条 asset:// 引用。'))
        } else {
          var assetTable = []
          for (var ai = assetRows.length - 1; ai >= 0 && assetTable.length < 20; ai -= 1) {
            var asset = asObject(assetRows[ai])
            if (asset === null) continue
            assetTable.push([
              React.createElement('span', { className: 'teamled-code', key: 'r' }, asText(asset.ref)),
              asText(asset.name) === '' ? '—' : asText(asset.name),
              asset.class === 'code' || asset.class === 'doc'
                ? React.createElement('span', { className: 'teamroster-prob-warn', key: 'c' }, asText(asset.class) + '（可进上下文）')
                : asText(asset.class),
              asText(asset.human) === '' ? String(asNumber(asset.bytes, 0)) + ' B' : asText(asset.human),
              asset.exists === false
                ? React.createElement('span', { className: 'teamroster-prob-err', key: 'e' }, '文件不在了')
                : stamp(asset.at),
            ])
          }
          assetNodes.push(rosterTable('assets', ['引用', '文件名', '类型', '大小', '收到时间'], assetTable))
        }
        assetNodes.push(React.createElement('div', { className: 'teamled-note', key: 'n' },
          '文件落在 <dataDir>/assets/<id>/<原名>，索引是 assets/index.jsonl（可以 tail）。' +
          '引用给的是**我们自己的盘**上的位置，不依赖飞书那条消息还在不在。'))
        children.push(React.createElement('div', { className: 'teamled-panel', key: 'assets' }, assetNodes))
      }

      /* ---- 团队文档与记忆（设计 01 §4 / 07 §0.3） ---- */
      var knowledge = asObject(payload.knowledge)
      if (knowledge !== null) {
        var docStats = asObject(knowledge.docs)
        var staleRows = asArray(knowledge.stale)
        var knowledgeNodes = []
        if (docStats === null || docStats.exists !== true) {
          knowledgeNodes.push(React.createElement('div', { className: 'teamled-note', key: 'none' },
            '工作区里还没有文档库（docs/）。用 team 工具的 docs 动作（op=init）按规范建一次，' +
            '之后 remember 沉淀的结论都会落在那里。'))
        } else {
          var byType = asObject(docStats.byType) ?? {}
          var typeBits = []
          var typeKeys = Object.keys(byType)
          for (var ki = 0; ki < typeKeys.length; ki += 1) typeBits.push(typeKeys[ki] + ' ' + String(asNumber(byType[typeKeys[ki]], 0)))
          knowledgeNodes.push(React.createElement('div', { className: 'teamled-row', key: 'h' },
            React.createElement('span', { className: 'teamled-panel-title', key: 't' }, '团队文档（' + String(asNumber(docStats.count, 0)) + '）'),
            React.createElement('span', { className: 'teamcfg-spacer', key: 'sp' }),
            React.createElement('span', {
              className: 'teamled-tag' + (staleRows.length > 0 ? ' teamled-tag-err' : ''),
              key: 'stale',
            }, staleRows.length > 0 ? '过期/被取代 ' + String(staleRows.length) : '没有过期的'),
          ))
          knowledgeNodes.push(React.createElement('div', { className: 'teamled-note', key: 'n' },
            (typeBits.length === 0 ? '（还没有文档）' : typeBits.join(' · ')) +
            ' · 缺字段 ' + String(asNumber(docStats.withProblems, 0)) + ' 份' +
            (nonEmptyString(docStats.latest) ? ' · 最近更新 ' + asText(docStats.latest) : '')))
          if (staleRows.length > 0) {
            var staleTable = []
            for (var si2 = 0; si2 < staleRows.length; si2 += 1) {
              var row2 = asObject(staleRows[si2])
              if (row2 === null) continue
              var reasons = asArray(row2.reasons).map(function (one) {
                var item = asObject(one)
                return item === null ? String(one) : asText(item.message)
              })
              staleTable.push([
                React.createElement('span', { className: 'teamled-code', key: 'i' }, asText(row2.id)),
                nonEmptyString(row2.owner) ? asText(row2.owner) : '（没有责任人）',
                asText(row2.updated) === '' ? '—' : asText(row2.updated),
                React.createElement('span', { className: 'teamroster-prob-warn', key: 'r' }, reasons.join('；')),
              ])
            }
            knowledgeNodes.push(rosterTable('stale', ['文档', '责任人', '最后更新', '为什么要看一眼'], staleTable))
          }
          knowledgeNodes.push(React.createElement('div', { className: 'teamled-note', key: 'n2' },
            '文档的主副本就是工作区里的 markdown（frontmatter 是硬要求，没有 owner 的写不进去）；' +
            'index.md 与 _meta/docs.json 都是**派生物**，重建即可。这里只显示"哪些该更新了"，' +
            '改内容请改文件本身。'))
        }
        children.push(React.createElement('div', { className: 'teamled-panel', key: 'knowledge' }, knowledgeNodes))
      }

      /* ---- 投递统计：降级率（设计 04 §11 第 4 条） ---- */
      if (delivery !== null) {
        /*
         * 这一格以前是把 delivery 的键值对直接 join 出来。加了分组统计之后
         * `byVia`/`byChat`/`byBot` 是对象，join 出的是「[object Object]」——
         * 所以这里改成真正的表：**降级率**要有数字，也要能看到是哪一级在发。
         */
        /*
         * 降级率的两个口径都要摆在页面上：**落盘**（每条消息最后落在哪一级，
         * 跨重启成立）与**本进程**（逐次投递，重启清零）。只显示前者会让人
         * 以为"降级率一直是 0"（刚重启），只显示后者会在重启后静默变好看。
         */
        var durable = asObject(delivery.durable)
        var fallbackRate = asNumber(durable === null ? delivery.fallbackRate : durable.fallbackRate, 0)
        var sentTotal = asNumber(durable === null ? delivery.total : durable.total, 0)
        var sessionRate = asNumber(delivery.sessionFallbackRate, asNumber(delivery.fallbackRate, 0))
        var deliveryNodes = []
        deliveryNodes.push(React.createElement('div', { className: 'teamled-row', key: 'h' },
          React.createElement('span', { className: 'teamled-panel-title', key: 't' }, '投递统计'),
          React.createElement('span', { className: 'teamcfg-spacer', key: 'sp' }),
          React.createElement('span', {
            className: 'teamled-tag' + (fallbackRate > 0.2 ? ' teamled-tag-err' : ''),
            key: 'rate',
          }, sentTotal === 0 ? '降级率 —（台账里还没有消息）' : '降级率 ' + (fallbackRate * 100).toFixed(1) + '%（落盘口径）'),
        ))
        var tiers = asArray(durable === null ? delivery.tiers : durable.tiers)
        if (tiers.length > 0) {
          var tierRows = []
          for (var ti = 0; ti < tiers.length; ti += 1) {
            var tier = asObject(tiers[ti])
            if (tier === null) continue
            var sentCount = asNumber(tier.sent, 0)
            tierRows.push([
              React.createElement('span', { className: 'teamled-code', key: 'v' }, asText(tier.via)),
              String(sentCount),
              sentTotal === 0 ? '—' : (sentCount / sentTotal * 100).toFixed(1) + '%',
              FALLBACK_VIAS.indexOf(asText(tier.via)) >= 0
                ? React.createElement('span', { className: 'teamroster-prob-warn', key: 'k' }, '降级')
                : '卡',
            ])
          }
          deliveryNodes.push(rosterTable('tiers', ['投递层级（via）', '发出条数', '占比', '类别'], tierRows))
        } else {
          deliveryNodes.push(React.createElement('div', { className: 'teamled-note', key: 'none' },
            '还没有成功投递过 —— 降级率要等第一条播报出去才有意义。'))
        }
        deliveryNodes.push(React.createElement('div', { className: 'teamled-note', key: 'n' },
          '这张表按**落盘的卡台账**数：一行台账 = 群里的**一条消息**，层级是它最后一次投递用的那一级。' +
          '「降级」= 卡没送出去、落到纯文本/文本加附件（' + FALLBACK_VIAS.join(' / ') +
          '）—— 降级率高说明卡片渲染有问题，不是消息发不出去。'))
        deliveryNodes.push(React.createElement('div', { className: 'teamled-note', key: 'n1' },
          '另一个口径（**本进程**，逐次投递、重启清零）：降级率 ' + (sessionRate * 100).toFixed(1) + '%，' +
          '总投递 ' + String(asNumber(delivery.total, 0)) + ' 次。两者差得远时通常是"刚重启"。'))
        deliveryNodes.push(React.createElement('div', { className: 'teamled-note', key: 'n2' },
          '@人 ' + String(asNumber(delivery.mentions, 0)) + ' 次' +
          '（配额拒绝后转静默 ' + String(asNumber(delivery.quotaRefused, 0)) + ' 次） · ' +
          '@配额/静默/节流/聚合 = ' + String(asNumber(delivery.suppressed, 0)) + ' / ' +
          String(asNumber(delivery.throttled, 0)) + ' / ' + String(asNumber(delivery.digested, 0)) +
          ' · 跳过 ' + String(asNumber(delivery.skipped, 0)) + ' · 失败 ' + String(asNumber(delivery.failed, 0))))
        var byBot = asObject(delivery.byBot)
        if (byBot !== null) {
          var botKeys = Object.keys(byBot)
          if (botKeys.length > 0) {
            var botRows = []
            for (var bi = 0; bi < botKeys.length; bi += 1) {
              var one = asObject(byBot[botKeys[bi]]) ?? {}
              botRows.push([
                React.createElement('span', { className: 'teamled-code', key: 'b' }, botKeys[bi]),
                String(asNumber(one.delivered, 0) + asNumber(one.updated, 0)),
                String(asNumber(one.mentions, 0)),
                String(asNumber(one.suppressed, 0) + asNumber(one.throttled, 0) + asNumber(one.digested, 0)),
              ])
            }
            deliveryNodes.push(rosterTable('bybot', ['机器人', '投递条数', '@人次数', '被静默'], botRows))
          }
        }
        deliveryNodes.push(React.createElement('div', { className: 'teamled-note', key: 'n3' },
          '下面这张表的口径是**本进程**（重启清零）：它们是"决定了不说"的行为计数，不是落盘对象。'))
        children.push(React.createElement('div', { className: 'teamled-panel', key: 'delivery' }, deliveryNodes))
      }

      /* ---- 每群发言占比（设计 04 §2.3 的 25% 告警 / §11 第 5 条） ---- */
      var share = asObject(payload.share)
      if (share !== null) {
        var shareRows = asArray(share.chats)
        var threshold = asNumber(share.threshold, 0.25)
        var shareNodes = []
        shareNodes.push(React.createElement('div', { className: 'teamled-row', key: 'h' },
          React.createElement('span', { className: 'teamled-panel-title', key: 't' }, '每群机器人发言占比'),
          React.createElement('span', { className: 'teamcfg-spacer', key: 'sp' }),
          React.createElement('span', { className: 'teamled-note', key: 'th' },
            '告警线 ' + (threshold * 100).toFixed(0) + '%（设计 04 §2.3）'),
        ))
        if (shareRows.length === 0) {
          shareNodes.push(React.createElement('div', { className: 'teamled-note', key: 'e' },
            '还没有任何群的消息记录（分子来自卡台账、分母再加上收件箱）—— 群里说过话之后这里才有行。'))
        } else {
          var shareTable = []
          for (var shi = 0; shi < shareRows.length; shi += 1) {
            var srow = asObject(shareRows[shi])
            if (srow === null) continue
            var shareValue = asNumber(srow.share, 0)
            var enough = srow.enough === true
            shareTable.push([
              React.createElement('span', { className: 'teamled-code', key: 'c' },
                nonEmptyString(srow.chatId) ? asText(srow.chatId) : '（没有 chat_id）'),
              String(asNumber(srow.sent, 0)),
              String(asNumber(srow.received, 0)),
              String(asNumber(srow.total, 0)),
              (shareValue * 100).toFixed(1) + '%',
              String(asNumber(srow.mentions, 0)),
              String(asNumber(srow.silenced, 0)),
              srow.alert === true
                ? React.createElement('span', { className: 'teamroster-prob-err', key: 'a' },
                  '超线告警')
                : enough
                  ? React.createElement('span', { className: 'teamroster-td-mute', key: 'a' }, '正常')
                  : React.createElement('span', { className: 'teamroster-td-mute', key: 'a' },
                    '样本不足（' + String(asNumber(srow.total, 0)) + '/' + String(asNumber(share.minSample, 8)) + '）'),
            ])
          }
          shareNodes.push(rosterTable(
            'share',
            ['群', '机器人发言', '人发言', '群消息总数', '占比', '@人次数', '被静默', '判定'],
            shareTable,
          ))
        }
        shareNodes.push(React.createElement('div', { className: 'teamled-note', key: 'n' },
          '分子 = 这个群里的卡台账行数（一张卡 = 群里的一条消息，原地更新不算新消息）；' +
          '分母 = 分子 + 收件箱里这个群的入站消息。两者都落盘，所以重启不会让比率变好看。' +
          '样本不足 ' + String(asNumber(share.minSample, 8)) + ' 条时不告警（新群第一句话就能超线，那是群刚建，不是刷屏）。'))
        shareNodes.push(React.createElement('div', { className: 'teamled-note', key: 'n2' },
          '超线时宿主会在定时扫描里写一行 warn 日志（按群冷却），所以这里只显示结论，不重复播报。'))
        children.push(React.createElement('div', { className: 'teamled-panel', key: 'share' }, shareNodes))
      }
    ...
    function TeamLedger() {
      var state = React.useState({
        ...
        pickedPrimary: {},
        /* 「日志」页：滚动数据，单独一条读取路径（见 LOGS_URL 的说明）。 */
        logsData: null,
        logsFetching: false,
        logsError: null,
        logsLevel: 'info',
      ...
      var load = function (options) {
        ...
        callApi('GET', undefined, GET_TIMEOUT_MS).then(function (outcome) {
          ...
          if (failure !== null) {
            ...
            /*
             * 404 是最需要解释的一种失败：**看不见的那一半可能压根没起来**。
             * 浏览器半边是静态脚本，宿主半边激活失败时面板照常渲染，于是屏幕上
             * 只剩一句"读取失败：HTTP 404" —— 看着像路由写错了，其实插件一行代码
             * 都没跑起来。entry 自己挂的 `/api/team/boot` 就是为这一刻准备的：
             * 探到原因，就把那句保守的说明换成真话。
             */
            if (outcome.status === 404) {
              bootExplanation().then(function (explanation) {
                if (explanation === null) return
                setState(function (previous) {
                  var hasData = previous.data !== null
                  var notice = asObject(previous.notice)
                  return Object.assign({}, previous, {
                    error: explanation,
                    notice:
                      hasData && notice !== null
                        ? Object.assign({}, notice, {
                          message: (isRetry === true ? '重试仍然失败：' : '') + explanation,
                        })
                        : previous.notice,
                  })
                })
      ...
      var loadConfig = function (options) {
        ...
        /*
         * `retry: true` = 用户点了「重新自检」：带 `?fresh=1` 绕过宿主的自检缓存，
         * 真的再打一次飞书（自检要调两个接口，各 15 秒上限，平时走 60 秒缓存）。
         */
        callApiTo(CONFIG_URL + (isRetry ? '?fresh=1' : ''), 'GET', undefined, GET_TIMEOUT_MS).then(function (outcome) {
          var payload = outcome.payload
          var failure = null
          if (outcome.error !== null) failure = outcome.error
          else if (payload === null || typeof payload !== 'object' || Array.isArray(payload)) failure = 'HTTP ' + String(outcome.status) + ' 的响应不是 JSON'
      ...
      /**
       * 读一次日志。**不碰配置快照**（那一份每读一次都要重算名册与自检），
       * 所以它有自己的一条 URL 与自己的相位。
       */
      var loadLogs = function (options) {
        /*
         * 筛选**必须由调用方带进来**，不能只从 `current` 读：点「warn」时
         * `patch()` 还没重渲染，`current.logsLevel` 仍是旧值 —— 于是那次请求
         * 带着旧条件出去，按钮看起来"点了没反应"（要等下一个自动刷新才对）。
         */
        var opts = options === null || typeof options !== 'object' ? {} : options
        var level = typeof opts.level === 'string' ? opts.level : current.logsLevel
        var source = typeof opts.source === 'string' ? opts.source : current.logsSource
        var query = '?level=' + encodeURIComponent(level) +
          '&source=' + encodeURIComponent(source) +
          '&limit=200&file=true'
        patch({ logsFetching: true })
        callApiTo(LOGS_URL + query, 'GET', undefined, GET_TIMEOUT_MS).then(function (outcome) {
          var payload = outcome.payload
          var failure = null
          if (outcome.error !== null) failure = outcome.error
          else if (payload === null || typeof payload !== 'object' || Array.isArray(payload)) failure = 'HTTP ' + String(outcome.status) + ' 的响应不是 JSON'
          else if (payload.ok !== true) failure = '接口返回了意外的结果'
          setState(function (previous) {
            var next = Object.assign({}, previous, { logsFetching: false })
            if (failure !== null) next.logsError = failure
            else {
              next.logsError = null
              next.logsData = payload
            }
            return next
          })
        })
      ...
      /* 自动刷新：只在「日志」页上跑，切走就停（免得在别的页上白刷）。 */
      React.useEffect(function () {
        if (current.tab !== 'logs' || current.logsAuto !== true) return undefined
        var timer = setInterval(function () { loadLogs({}) }, LOGS_REFRESH_MS)
        return function () { clearInterval(timer) }
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [current.tab, current.logsAuto, current.logsLevel, current.logsSource])

      /* 切到「日志」页时先取一次（它不在配置快照里）。 */
      React.useEffect(function () {
        if (current.tab !== 'logs') return undefined
        if (current.logsData !== null) return undefined
        loadLogs({})
        return undefined
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, [current.tab, current.logsData])
      ...
      if (data !== null) {
        ...
        for (i = 0; i < requirements.length; i += 1) {
          ...
          var currentReqId = requirement === null ? '' : asText(requirement.id)
          ...
          /*
           * 需求的动作行。按钮来自 host 的 `available`（状态机允许什么就画什么），
           * 面板不再自己维护一份状态表 —— 那种"两处各写一遍规则"迟早会不一致。
           */
          var reqAvailable = readTextList(requirement.available)
          var reqActionButtons = []
          for (var ra = 0; ra < REQ_ACTION_ORDER.length; ra += 1) {
            var reqAction = REQ_ACTION_ORDER[ra]
            if (reqAvailable.indexOf(reqAction) < 0) continue
            reqActionButtons.push(React.createElement('button', {
              className: 'teamled-btn',
              type: 'button',
              key: 'reqact-' + reqAction,
              disabled: isBusy || canWrite !== true,
              title: 'POST action=' + REQ_ACTION_CALL[reqAction],
              onClick: function (actionName) {
                return function () { runAction(REQ_ACTION_LABEL[actionName], REQ_ACTION_CALL[actionName], { id: currentReqId }) }
              }(reqAction),
            }, REQ_ACTION_LABEL[reqAction]))
          }
          if (reqActionButtons.length > 0) {
            reqCard.push(React.createElement('div', { className: 'teamled-actions', key: 'reqactions' }, reqActionButtons))
          }
          if (reqAvailable.indexOf('change') >= 0 || reqAvailable.indexOf('drop') >= 0) {
            reqCard.push(line('reqnote', 'teamled-note',
              '「变更」与「废弃」要写原因，走 `team` 工具（action=change_requirement / drop_requirement）：这一页没有理由输入框，与其静默丢掉理由，不如不画那个按钮。'))
          }
        ...
        for (i = 0; i < tasks.length; i += 1) {
          ...
          for (var a = 0; a < ACTION_ORDER.length; a += 1) {
            ...
            actionButtons.push(React.createElement('button', {
              ...
              key: key,
              disabled: isBusy || canWrite !== true,
              title: 'POST action=' + ACTION_CALL[key] + (key === 'submit' ? '（附一条"人工提交"的说明，不是真证据）' : ''),
              onClick: function (actionKey, actionName, taskId) {
                return function () {
                  runAction(ACTION_LABEL[actionKey], actionName, actionKey === 'submit'
                    /*
                     * 提交验收要求"有证据"（状态机硬闸），而这一页没有证据输入框。
                     * 所以这里写的是一条**如实说明自己没有证据**的备注，而不是假装
                     * 有一份可以核对的东西 —— 验收人点开就会看到这句话。
                     */
                    ? { id: taskId, evidence: { kind: 'note', ref: 'gui:' + String(Date.now()), note: '（人工在面板上提交，没有附证据；证据请看执行会话汇报或任务卡上的其他条目）' } }
                    : { id: taskId })
                }
      ...
      var tabsNode = React.createElement('div', { className: 'teamled-tabs', key: 'tabs' },
        ...
        tabButton('logs', '日志', '刚才发生了什么：分级日志（含重启前的）+ 漏单与分诊结论 + 投递统计'),
      )
      ...
      else if (current.tab === 'logs') {
        rosterNode = TeamLogsPage({
          payload: current.logsData,
          fetching: current.logsFetching === true,
          level: current.logsLevel,
          source: current.logsSource,
          autoRefresh: current.logsAuto === true,
          error: current.logsError,
          onLevel: function (value) { patch({ logsLevel: value }); loadLogs({ level: value }) },
          onSource: function (value) { patch({ logsSource: value }); loadLogs({ source: value }) },
          onAuto: function (value) { patch({ logsAuto: value === true }) },
          onReload: function () { loadLogs({}) },
        })
      }
