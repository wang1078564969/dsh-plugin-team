...
export function createDocStore(deps) {
  ...
  function write(input) {
    ...
    return put(file, frontmatter, body, existing === null)
  }

  /** 落盘（原子写：先写 `.tmp` 再 rename，写坏不会留下半个文件）。 */
  function put(file, frontmatter, body, isNew) {
    try {
    ...
    return { ok: true, id: frontmatter.id, path: relative(workspace, file), file, frontmatter, created: isNew }
  }

  /**
   * **人确认一份草稿**（R10.4）：`draft → active`。
   *
   * 机器人沉淀的知识默认落 `draft`（"幻觉不该直接进主副本"），但一份永远转不了正的
   * 草稿等于没有沉淀 —— 这里补上那个一直缺的动作。三条硬要求：
   *
   *   1. **必须有确认人**（`by`）：谁点头的写进 `confirmed_by`，不写就不改状态；
   *   2. **只从 `draft` 出发**：已经 active 的再来一次是 `invalid_state`，
   *      而不是"悄悄再写一遍"（状态跃迁只有一条路，和台账那边同一个规矩）；
   *   3. **正文不动**：确认是元数据动作，不碰内容 —— 人再看时，正文还是他读过的那份。
   *
   * @param {string} id
   * @param {{by?: string}} [input]
   * @returns {{ok: boolean, code?: string, message?: string, path?: string, frontmatter?: object}}
   */
  function confirm(id, input = {}) {
    const by = String(input.by ?? '').trim()
    if (by === '') return { ok: false, code: 'bad_request', message: '确认文档要写清 by：谁确认的' }
    const doc = read(String(id))
    if (doc === null) return { ok: false, code: 'not_found', message: '没有这份文档：' + String(id) }
    if (doc.status !== 'draft') {
      return {
        ok: false,
        code: 'invalid_state',
        message: '这份文档现在是 ' + String(doc.status) + '，只有 draft 需要确认（要改状态就用 op=write）',
      }
    }
    const day = today(now)
    const frontmatter = {
    ...
    const checked = validateFrontmatter(frontmatter)
    if (checked.ok !== true) return { ok: false, code: 'invalid_frontmatter', problems: checked.problems }
    return put(doc.file, frontmatter, doc.body, false)
  }
  ...
  return { workspace, root, init, list, read, write, confirm, search, references, staleReport, rebuildIndex, stats }
}
