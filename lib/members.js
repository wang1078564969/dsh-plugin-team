...
/**
 * 这个人现在能不能做写操作 —— **成员表第一次真的拦人**。
 *
 * 设计 02 §2 与 05 §3.2 的三档：`owner`（治理）/`member`（做事）/`observer`（只看）。
 * 以前成员表只提供显示用的姓名与角色域，`role` 与 `canApprove` 零消费者 ——
 * 于是"观察者不能改台账"这句话在代码里没有任何落点。
 *
 * 三条规则，都是"拒绝的时候说得清为什么"：
 *   1. `observer` 一律只读；
 *   2. `active: false` 的人只能通过 `delegate` 代做（否则他在休假，活却挂他名下）；
 *   3. `canApprove` 非空时，它是一个**白名单**：验收这类动作要求名单里有对应项。
 *      留空 = 不做限制（默认不打扰：老配置里没人填过这个字段）。
 *
 * @param {object} resolved `resolveMembers()` 的结果
 * @param {{actor: string, action: string, needsApproval?: string}} input
 * @returns {{ok: true, via: string}|{ok: false, message: string}}
 */
export function memberCanWrite(resolved, input) {
  const actor = typeof input?.actor === 'string' ? input.actor : ''
  const member = resolved?.byKey?.get(actor) ?? null
  /*
   * 不在成员表里的人：
   *   · **名册还是空的** → 不限制。一个还没建立成员表的安装如果连写操作都拒，
   *     第一个人什么都做不了；
   *   · **名册非空** → 拒绝。名册一旦建立，它就该是一张白名单 ——
   *     否则"把某人设成 observer"防不住他换一个没登记的主体继续写。
   */
  if (member === null) {
    /*
     * **成员表管人，不管机器人** —— 这条分界很重要：
     *   · `bot:*` / `role:*` / `system` 是机器人侧，它们的能力与作用域由
     *     `lib/bots.js` 的 `botCan` / `botCanTouchRepo` 判（那是另一层闸，
     *     而且名册里本来就不会有机器人行）；
     *   · `human:*` 才是这张表管的。名册**空**时不限制（还没建名册的安装
     *     不能谁都动不了）；名册**非空**时它就是白名单 —— 否则"把某人设成
     *     observer"防不住他换一个没登记的主体继续写。
     */
    if (!String(actor).startsWith('human:')) return { ok: true, via: 'bot-side' }
    const known = Array.isArray(resolved?.list) ? resolved.list.length : 0
    if (known === 0) return { ok: true, via: 'no-roster' }
    return {
      ok: false,
      code: 'not_a_member',
      message: actor + ' 不在成员表里（这张表有 ' + String(known) + ' 个人）：先在「成员」页把他加进去，再让他动手',
    }
  }
  if (member.role === 'observer') {
    return { ok: false, message: member.name + ' 是观察者（observer）：只能看台账，不能改' }
  }
  if (member.active === false) {
    const delegate = typeof member.delegate === 'string' && member.delegate !== '' ? member.delegate : null
    if (delegate === null) {
      return { ok: false, message: member.name + ' 已停用且没有代理：他名下的活先交给别人或加一个 delegate' }
    }
    return { ok: false, message: member.name + ' 已停用，请由代理 ' + delegate + ' 来操作' }
  }
  const needs = typeof input?.needsApproval === 'string' ? input.needsApproval : null
  if (needs !== null && member.canApprove.length > 0 && !member.canApprove.includes(needs)) {
    return {
      ok: false,
      message: member.name + ' 的 canApprove 里没有 ' + needs + '（现有：' + member.canApprove.join('/') + '）',
    }
  }
  return { ok: true, via: member.role }
}
