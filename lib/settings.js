...
export function validatePatch(patch, context = {}) {
  ...
  if (patch.feishu !== undefined) {
    ...
    if (feishu === null || typeof feishu !== 'object') bad('feishu', '必须是一个对象')
       ...
       */
      /*
       * `feishu.buttons: true` 是**假承诺**：卡片按钮要生效，应用必须在飞书侧配置一个
       * 公网 HTTPS 回调地址，而且插件要注册 `card.action.trigger` —— 这两件事本插件
       * 都没有（连接只注册了 `im.message.receive_v1` 与入群事件，DSH Web 也只听 127.0.0.1）。
       * 打开它的结果是：群里出现一排点下去毫无反应的按钮，而且卡片不再给出文本指令
       * （第一级用不着降级），求助无门。所以这里直接拒绝，并说清替代路径。
       */
      if (feishu.buttons === true) {
        bad(
          'feishu.buttons',
          '本版本没有卡片回调端点（插件不注册 card.action.trigger，DSH 也只监听 127.0.0.1）：' +
            '打开它只会得到点了没反应的按钮。确认动作走群文本命令（卡片上会写明回复哪一句）',
        )
      }
      ...
      for (const key of ['requireMention', 'respond', 'buttons', 'addressedOverridesIntent', 'requireRegisteredChat', 'reaction']) {
        if (feishu[key] !== undefined && typeof feishu[key] !== 'boolean') bad('feishu.' + key, '必须是 true / false')
      }
      /*
       * 表态的表情与范围。
       *
       * 表情名**不校验它是不是飞书认识的那一批之一**：那份清单会变，写死一份副本
       * 只会在飞书加了新表情之后拒绝一个完全合法的值。写错的结果是接口报错 —— 而那条
       * 错误会出现在日志页的 `reaction` 行里（`表态失败：…`），比在这里猜要诚实。
       * 能校验的是**形状**：非空字符串。
       */
      if (feishu.reactionEmoji !== undefined && (typeof feishu.reactionEmoji !== 'string' || feishu.reactionEmoji.trim() === '')) {
        bad('feishu.reactionEmoji', '填飞书表情的英文名，例如 Get（收到）/ DONE / OK / OnIt；要表态就得填一个')
      }
      if (feishu.reactionScope !== undefined && feishu.reactionScope !== 'all' && feishu.reactionScope !== 'addressed') {
        bad('feishu.reactionScope', '只能是 all（每一条收到的消息）或 addressed（只对 @ 了机器人的消息）')
      }
