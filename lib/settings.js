...
export const KNOWN_DOMAINS = ['pm', 'requirement', 'development', 'testing', 'ops', 'security']
...
export function validatePatch(patch, context = {}) {
  ...
  if (patch.feishu !== undefined) {
    ...
    if (feishu === null || typeof feishu !== 'object') bad('feishu', '必须是一个对象')
      ...
      for (const key of ['requireMention', 'respond', 'buttons', 'addressedOverridesIntent']) {
...
export function saveConfig(input) {
  ...
  try {
    if (backup !== null) writeFileSync(backup, readFileSync(input.configFile), { mode: 0o600 })
