 ...
 */
/** 会真的动代码的任务类型：只有它们才需要分支与 trailer 约定。 */
const CODE_TYPES = new Set(['code_change', 'defect_fix', 'feature_delivery', 'new_feature', 'data_change', 'refactor'])

/**
 * 代码类任务的分支与提交约定（设计 03 §1.3）。
 *
 * 写进 prompt 而不是"靠平台功能"：这个形态下没有 webhook、没有能点的按钮，
 * **代码历史与需求对象的双向可追就靠这两行文本**。`git log` 里能看到这个分支为哪个
 * 需求而生；需求卡上能看到涉及哪些分支。
 */
function gitConvention(task, requirement) {
  const reqId = String(requirement?.id ?? task.req ?? '')
  if (reqId === '') return []
  const slug = String(requirement?.title ?? task.title ?? '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 40)
  const base = 'req/' + reqId + (slug === '' ? '' : '-' + slug)
  const branch = reqId === '' ? base : 'req/' + reqId + '/' + String(task.id)
  return [
    '',
    '代码改动按团队约定（设计 03 §1.3）：',
    '  - 需求分支：' + base,
    '  - 这个任务的分支：' + branch,
    '  - **每条提交**的信息末尾带上 trailer：`Req: ' + reqId + '` 与 `Task: ' + String(task.id) + '`',
    '    （`git log` 靠它们反查这条提交属于哪个需求；漏了双向可追就断了）',
    '  - 开 MR 之后把链接交给团队层记账（`team` 工具的 `repo op=link_mr`），不要自己合并',
  ]
}

export function taskPrompt(task, requirement, extra = {}) {
  const gitLines = CODE_TYPES.has(String(task.type ?? '')) ? gitConvention(task, requirement) : []
  const lines = []
  ...
  for (const line of gitLines) lines.push(line)
