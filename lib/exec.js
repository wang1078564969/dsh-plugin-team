...
export class SessionPool {
   ...
   * @returns {Promise<{text: string, timedOut: boolean, seq: number}>}
   */
  async drive(agent, text, options = {}) {
    ...
    if (session !== undefined && typeof session.snapshotEvents === 'function') {
      try {
        collected = lastAssistantText(session.snapshotEvents(startSeq))
      } catch (error) {
        collected = ''
      }
    }
    return { text: collected, timedOut, seq: startSeq }
  }
