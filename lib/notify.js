...
/**
 * 今天（**本地**日期）作为 @人配额的分桶键 —— 本模块自己不读时钟，日期由调用方给。
 *
 * 用本地而不是 UTC：配额是"每天"的概念，而人的一天按本地时区算。
 * 用 UTC 的话，UTC-6 及以西的部署会在下午就把配额重置掉（那正是人最活跃的时候）。
 */
...
export function createNotifier(deps) {
  ...
  /*
   * 配额落盘（R13.5）：`store` 顺手带上，并在这里算一次"今天是几号" ——
   * `MentionQuota` 自己不读时钟（否则它没法被测），但"重启之后还算不算今天"必须有人回答。
   */
  ...
  async function deliver(input) {
    ...
    if (decision.mode === 'digest') {
      /*
       * 桶键 = `群|类型`。日报是**按群**发的（每个群看自己的摘要），所以桶必须
       * 先按群分开；不分类的话，A 群的进度会出现在 B 群的日报里。
       * 没有群的（DSH 里建的对象）归到 `no-chat` 桶，照样不丢。
       */
      const bucket = digest.add(decision, { line: input.line, topic: (chatId ?? 'no-chat') + '|' + String(decision.digest_bucket ?? 'misc') })
    ...
    if (chatId === null) {
      ...
      /*
       * 留一行日志。这一类跳过（在 DSH 里用工具建的需求没有来源群）**不是错误**，
       * 但"为什么群里没动静"是运维最常问的问题 —— 不记的话，答案只能靠读代码。
       */
      log.info?.('[team] 没有来源群，这条不播报（reason=' + String(decision.reason) + '）：' + String(input.line ?? ''))
  ...
  /**
   * 把决策里"要 @ 谁"变成**真的会通知人**的一行。
   *
   * 设计 04 §2.3 说"谁没确认就 @ 谁"，而飞书里只有 `<at id=ou_x></at>` 才会通知 ——
   * 手写"@张三"只是一段文字。open_id 从成员表拿（`members[].openId`）；
   * **拿不到就退回名字**：一个查不到 open_id 的成员，写名字至少人看得懂是谁，
   * 编一个 id 则会让整条消息发不出去。
   */
  function atLine(principals) {
    const list = Array.isArray(principals) ? principals : []
    const parts = []
    for (const who of list) {
      const principal = String(who ?? '')
      if (principal === '') continue
      /*
       * 读**归一化之后**的名册（`config.memberList`）：`config.members` 在老形态下
       * 是 `{域: [人]}` 的对象，对它调 `.find` 会抛异常 —— 而这里在 try 里，
       * 结果是"卡片构造失败、什么都不播报"，症状离原因很远。
       */
      const roster = Array.isArray(config.memberList) ? config.memberList : []
      const member = roster.find((one) => one !== null && typeof one === 'object' && one.key === principal)
      const openId = typeof member?.openId === 'string' ? member.openId : ''
      parts.push(openId === '' ? '@' + shortName(principal) : '<at id=' + openId + '></at>')
    }
    return parts.join(' ')
  }
  ...
  function cardCtx() {
    return {
      ...
      /* 上一轮执行的耗时 / 步数 / token（`run_task` 写在会话记录上）。 */
      runOf: (taskId) => store.get('session', String(taskId))?.last_run ?? null,
  ...
  return {
    ...
    task(task, opts = {}) {
      ...
      try {
        ...
        /*
         * @人行加在**配额之后**的名单上：被降级成"静默待办"的人不该出现在这里，
         * 否则配额只是不计数，人还是被叫了一遍。
         */
        const line = atLine(decision.mention)
        if (line !== '') card = { ...card, at_line: line }
      } catch (error) {
     ...
     * 一次性通知（催办、升级、异常）。
     *
     * ⚠️ **同一个 `id` 的第二次调用是原地更新（PATCH），不是又一条新消息** ——
     * `card_key` 是 `callout:<id>`，幂等就靠它。所以"必须再提醒一次"的场景
     * （比如每小时催一次到期的门禁）**要让 id 带时间片**，否则第二次只是把旧卡
     * 改了一下，人根本不会收到通知。`lib/tools.js` 的 tick 催办就是这么做的。
     */
    notice(opts) {
      ...
      try {
        card = buildNoticeCard({
          ...(atLine(decision.mention) === '' ? {} : { at_line: atLine(decision.mention) }),
        title: opts.title,
    ...
    report(opts) {
      ...
      /*
       * 与 `task`/`notice` 同一条规矩：**卡片构造失败不能变成抛异常**。
       * 这里以前没有 try，而 `buildReportCard` 会对 `lines` 调 `.join` ——
       * 一次形状不对的调用会让日报的按群循环当场中断，**当天后面所有群都不发**，
       * 而错误只出现在一个没人看的 promise 里。`lines` 非数组时退化成空数组。
       */
      let card = null
      try {
        card = buildReportCard({
          title: opts.title,
          lines: Array.isArray(opts.lines) ? opts.lines.map((one) => String(one)) : [],
        })
      } catch (error) {
        log.error?.('[team] 报告卡构造失败（这一条不发）：' + String(error && error.message ? error.message : error))
        return { action: 'skip', reason: 'card-build-failed', card_key: null }
      }
     ...
     * 取走全部摘要（日报用）。
     *
     * 与 `digestBuckets()`（只读）分开：日报发出去之后桶要清空，
     * 否则同一条进度今天、明天各说一次。
     */
    drainDigest() {
      return digest.drain()
    ...
    /**
     * @人配额的当日明细（R13.5）：谁被 @ 了几次、有几次被改成静默待办。
     *
     * 从**落盘的计数**读，不是从本进程的计数器读 —— 后者重启就清零，
     * 而"配额到底有没有拦住人"这件事必须跨重启可查。
     */
    quota: (day = dayKeyOf(now())) => quota.rows(day),
  }
}
