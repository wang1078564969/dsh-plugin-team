...
export const KINDS = {
  ...
  /**
   * 一次执行轮次的用量事实：`{ day, bot, chat, task, req, tokens, elapsed_ms, steps }`。
   *
   * 为什么值得落盘：卡片 footer 只回答"上一轮花了多少"，而"这个机器人这个群这些天
   * 花了多少"是钱的问题，钱的问题不能只活在内存里（R13.6）。一条一轮、不聚合 ——
   * 聚合是读的时候算的（`lib/metrics.js` 的 `usage()`），落盘只存事实。
   */
  usage: { dir: 'usage', prefix: 'usage-', idStyle: 'seq' },
  /**
   * @人配额的一天一人一行（R13.5）：`{ id, day, bot, who, used, silenced }`。
   *
   * id 由 `day|bot|who` 拼出来（`|` 与 `:` 都不是文件安全字符，见 `quotaId()`），
   * 原始字段照旧存在记录里。落盘的理由很直白：配额管的是"今天还能 @ 你几次"，
   * 重启就清零等于没有配额 —— 而它防的正是"一个下午被 @ 二十次"。
   */
  quota: { dir: 'quota', prefix: 'quota-', idStyle: 'external' },
}
