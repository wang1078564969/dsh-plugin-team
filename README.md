...
| **会话** | 每个机器人 × 每个群的会话：群 / DSH 会话 id / 轮次 / 最后活动 / 工作区 |
...
- **多人验收门禁实际走不通（hub 既有 bug，移植时按原样保留）**：`verify` 在
  `required_by` 有两人时，`gate_incomplete` 会把已记录的那条确认一起丢掉，第二个
  人再点又差第一个 —— 永远 ping-pong。`acceptors()` 的语义看起来是"任一域负责人
  可验收"，但 `gateSatisfied` 要求全部确认。当前实现把它钉在测试里，**改法是产品决策**。
