...
| `test/` | ✅ 527 个用例全绿（`npm test`），另有 4 个**可选**的渲染测试（见「实测」第 8 条） |
...
本地测试：`npm test` → **511 passed / 0 failed / 16 skipped**（跳过的是**可选**渲染测试组：台账页 / 配置页 /
机器人·成员·会话三页 / 日志页，`npm i -D react react-dom jsdom` 后即跑 → **527 passed / 0 failed / 0 skipped**）。
